...
#for HC1kGP3 - use per-chromosome version as the full version takes a very long time
...
sbatch --time 2-00:00:00 --partition cpu --job-name="varlist" --ntasks 1 --cpus-per-task 4 --mem 40G --wrap="Rscript /users/k19049801/project/JZ_GED_PHD_C1/scripts/create_variant_list.R" --output "create_variant_list.$(date +%Y%m%d).out.txt"
