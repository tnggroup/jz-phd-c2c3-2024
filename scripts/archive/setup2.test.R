...
if(!is.null(project$lfGWAS$sumstats)){
  ...
  if (!file.exists(file.path(project$folderpath.workingDirectory,paste0("lfGWAS.gwas.",project$setup.code,".test.Rds"))) | project$setting.refreshLatentFactorGWAS) 
  {
  ...
  print("DONE performing latent factor GWAS. The results should have been saved to a file.")
    ...
    project$lfGWAS$gwas<-readRDS(file=file.path(project$folderpath.workingDirectory,paste0("lfGWAS.gwas.",project$setup.code,".test.Rds")))
  }
...
for(igwas in 1:length(project$lfGWAS$gwas)){
  ...
  if(length(which(project$sumstats.sel$code==fcode))<1){
    ...
    project$sumstats.sel[which(project$sumstats.sel$code==fcode),c("mungedpath")]<-file.path(project$folderpath.workingDirectory,fcode)
  }
