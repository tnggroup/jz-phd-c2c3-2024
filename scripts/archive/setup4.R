...
project$CFA$models<-data.frame(nModel=c(),code=c(),nFactors=c(),lModel=c(),lResults=c())
  ...
  for(iFactorConfiguration in 1:length(project$CFA$nFactors)) {
    ...
    for(nSessioPattern in 1:sessionPatternLength){
      ...
      project$CFA$models[nModel,c("nModel")]<-nModel
        ...
        #code
      project$CFA$models[nModel,c("code")]<-paste0("M",project$CFA$nIndicators,"-",project$CFA$nFactors[iFactorConfiguration],
      ".",nSessioPattern,
...
project$CFA$models.selected<-project$CFA$models[c("M25-4.74.ML","M25-7.29.ML","M25-10.55.ML"),] #this is a manual setting after choosing the best fitting models from the previous results 
...
for(iSelected in 1:nrow(project$CFA$models.selected)){
  ...
  project$CFA$models.selected[[iSelected,c("lModel.fixed")]]<-semplate$generateLavaanCFAModel(
    allow_loading.table.indicator_factor = cIndicatorLoadings,
    fix_loading.table.indicator_factor = project$CFA$models.selected[iSelected,]$parsedGsemResults[[1]][[1]]$patternCoefficientsSTDGenotype.matrix, fix_loading.table.residual_variance = project$CFA$models.selected[iSelected,]$parsedGsemResults[[1]][[1]]$residualVaraiancesSTDGenotype.matrix,
    orthogonal = T)
...
if(!is.null(project$lfGWAS$sumstats)){
  ...
  #test 
  #project$clOptions$task_argument<-"M25-4.74.ML:1"
  #project$clOptions$task_argument<-"M25-4.74.ML"
  ...
  if(project$clOptions$task=="lfgwas" & !file.exists(file.path(project$folderpath.workingDirectory,paste0("lfGWAS.gwas.",project$setup.code,".Rds")))) 
  {
    ...
    cat("\nSelected factor ",ifelse(is.null(project$lfGWAS$cFn),"ALL",paste0(project$lfGWAS$cFn))," and chromosome ",paste0(project$lfGWAS$cChr),". This will take a while!\n")
