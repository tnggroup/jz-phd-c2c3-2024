...
hdl.original_scratch<-function(traits,sample.prev,population.prev,trait.names,LD.path,jackknife){
...
```{r display ld rg, fig.width=9, fig.height=6, out.width="1000px", out.height="1200px", purl=FALSE}
...
project$functions$plot.rg.dot.multi<-function(S,S.se,S.std,S.std.se,plotTitle,plotSubtitle){
  ...
  p1<-project$functions$plot.rg.dot(df = df, rg = df$rg, xvar = df$h.mean, rg.se = df$rg.se, xlab="Mean trait SNP heritability, liability scale")
  
  p2<-project$functions$plot.rg.dot(df = df, rg = df$rg, xvar = df$h.absdiff, rg.se = df$rg.se, xlab="Absolute difference in trait SNP heritability, liability scale")
  
  p3<-project$functions$plot.rg.dot(df = df, rg = df$rg, xvar = df$rg.se, rg.se = df$rg.se, xlab="SNP-based genetic correlations, standard error, liability scale")
...
plotRg.mvLDSC<-project$functions$plot.rg.dot.multi(
  S = project$mvLD$covstruct.HDL.jackknife,
  S.se = project$mvLD$covstruct.mvLDSC$S.SE,
  S.std = project$mvLD$covstruct.mvLDSC$S_Stand,
  S.std.se = project$mvLD$covstruct.mvLDSC$S_Stand.SE,
  plotTitle = "mvLDSC",
