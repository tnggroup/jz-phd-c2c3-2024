...
for(iSelected in 1:nrow(p$CFA$models.selected)){
  ...
  #set nice rownames
  df$var <- p$sumstats.sel[df$var, ]$name.nice.and_code
  #df<-df[order(df$var, decreasing = T,method = "radix"),]
  df$var <- as.factor(df$var)
  
  df.l <- pivot_longer(data = df,  -var)
  #df.l<-df.l[order(df.l$name,df.l$var, decreasing = c(F,F),method = "radix"),]
  
  #ggplot(df.l, aes(x = var, y = abs(value), fill=value)) + 
...
#set nice names
names(codesAllSets) <- p$sumstats.sel[names(codesAllSets), ]$name.nice.and_code

#png(filename = file.path(p$folderpath.plots,paste0("ga.png")), width = 1200, height = 1200, units = "px")
...
upset(
  ...
  text.scale = c(1, 0.7, 1, 1, 0.8, 0.6) #c(intersection size title, intersection size tick labels, set size title, set size tick labels, set names, numbers above bars)
  )
