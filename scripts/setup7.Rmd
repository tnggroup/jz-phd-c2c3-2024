...
#add settings on whether to force a new N or just complete the N
p$sumstats$forceN<-F #standard is to just complete missing N's
p$sumstats["ANXI04",]$forceN<-T #dataset has a wrong N overall
...
p$sumstats.sel[,c("code","name","name.nice","name.nice.and_code", "year","n_case","n_control","n_total","pmid","reference_doi","samplePrevalence","populationPrevalence","dependent_variable.OLS","dependent_variable.linprob","se.logit","mungedpath","forceN")]
...
if(file.exists(p$filepath.lfgwas.sumstats)) {
  ...
  # colSE<-colnames(p$lfGWAS$sumstats)[grep("^SE\\.", ignore.case = TRUE,colnames(p$lfGWAS$sumstats))]
  # 
  # #cSumstats<-p$lfGWAS$sumstats[1:10000,]
  # 
  # for (i in colBeta)
...
# 
# p$printCorr(
#   corr = (p$mvLD$covstruct.mvLDSC.1kg$S_Stand - p$mvLD$covstruct.mvLDSC.hm3$S_Stand),
#   SE = NULL,
...
# 
# p$printCorr(
#   corr = (p$mvLD$covstruct.mvLDSC.1kg$S_Stand.SE - p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE),
#   SE = NULL,
...
#p$clOptions$task_argument<-"M25_7_26.ORT.ML:5"
#p$clOptions$task_argument<-"M25_7_26.ORT.ML"
...
if(!is.null(p$lfGWAS$sumstats)){
  
  cat("\nChromosomes in lfGWAS sumstats:\n",unique(p$lfGWAS$sumstats$CHR),"\n")
  
  #test 
  #p$clOptions$task_argument<-"M25_4_27.COR.ML"
  ...
  if(p$clOptions$task=="lfgwas" & !file.exists(file.path(p$folderpath.workingDirectory,paste0("lfGWAS.gwas.",p$setup.code,".Rds")))) 
  {
    ...
    if(length(p$lfGWAS$cChr)>0){
      ...
      p$lfGWAS$sumstats.selected<-p$lfGWAS$sumstats[which(as.character(p$lfGWAS$sumstats$CHR)==p$lfGWAS$cChr),]
    } else {
      p$lfGWAS$sumstats.selected<-p$lfGWAS$sumstats
    }
      
    p$lfGWAS$gwas<-userGWAS.mod(
      covstruc = p$mvLD$covstruct.mvLDSC,
      SNPs = p$lfGWAS$sumstats.selected,
      estimation = "DWLS",
...
for(iModel in 1:nrow(p$CFA$models.selected)){
  ...
  if(!is.null(p$lfGWAS$gwas[[cModel$code]])){
    ...
    for(iFactor in 1:cModel$nFactors){
      ...
      gwasForEffectiveN<-subset(p$lfGWAS$gwas[[cModel$code]][[iFactor]], p$lfGWAS$gwas[[cModel$code]][[iFactor]]$MAF <= .4 & p$lfGWAS$gwas[[cModel$code]][[iFactor]]$MAF >= .1)
      ...
      p$lfGWAS$gwas[[cModel$code]][[iFactor]]$N<-effectiveN[iFactor]
    }
...
for(iModel in 1:length(p$CFA$models.selected)){
  #test
  #iModel<-4
  ...
  for(iFactor in 1:cModel$nFactors){
    #test
    #iFactor<-5
    p$lfGWAS$gwas.for.display.filename<-file.path(p$folderpath.workingDirectory,paste0("nmeta.",cModel$code,".F",iFactor,".",p$setup.code,".Rds"))
    if(!file.exists(p$lfGWAS$gwas.for.display.filename)) next
    
    p$lfGWAS$gwas.for.display <- na.omit(readRDS(p$lfGWAS$gwas.for.display.filename))
    p$lfGWAS$gwas.for.display <- p$lfGWAS$gwas.for.display[,Z:=BETA/SE][FRQ>=0.01] ##ONLY COMMON VARIANTS
    ...
    gtsave(data = cTable, filename = file.path(p$folderpath.plots,cModel$code,paste0("top.F",iFactor,".rtf")))
    ...
    #manhattan plot 2
    png(filename = file.path(p$folderpath.plots,cModel$code,paste0("man.F",iFactor,".png")), width = 1200, height = 600)
    qqman::manhattan(x = qqmanGwas)
    dev.off()
    
    #qqman::qq(p$lfGWAS$gwas.for.display$Pval_Estimate)
    
    png(filename = file.path(p$folderpath.plots,cModel$code,paste0("qq.F",iFactor,".png")), width = 600, height = 600)
...
for(iModel in 1:length(p$lfGWAS$gwas)){
  #test
  #iModel<-3
  
  cModel<-p$CFA$models.selected[iModel,]
  lfGwasList <- p$lfGWAS$gwas[[iModel]]
...
p$sumstats.lfgwas<-data.frame(code=NA, mungedpath=NA)
for(iModel in 1:length(p$lfGWAS$gwas)){
  #test
  #iModel<-1
  cModel<-p$CFA$models.selected[iModel,]
  ...
  for(iGwas in 1:length(lfGwasList)){
    #test
    #iGwas<-1
    if(is.na(p$sumstats.lfgwas$code[gwasNames[iGwas]])){
      p$sumstats.lfgwas[[gwasNames[iGwas],c("code")]]<-gwasNames[iGwas]
    }
    
    p$sumstats.lfgwas[[gwasNames[iGwas],c("mungedpath")]]<-file.path(p$folderpath.data.sumstats.munged,paste0(gwasNames[iGwas],".gz"))
    
    
  }
}
#remove the pesky first empty row
p$sumstats.lfgwas<-p$sumstats.lfgwas[which(!is.na(p$sumstats.lfgwas$code)),]

p$sumstats.sel.lfgwas<-rbindlist(list(p$sumstats.sel,p$sumstats.lfgwas),use.names = T, fill = T)
rownames(p$sumstats.sel.lfgwas)<-p$sumstats.sel.lfgwas$code

p$sumstats.sel.lfgwas$lavaan_code<-str_replace_all(p$sumstats.sel.lfgwas$code,"-","_")

p$sumstats.sel.lfgwas[which(is.na(p$sumstats.sel.lfgwas)),c("name.nice")]<-p$sumstats.sel.lfgwas[which(is.na(p$sumstats.sel.lfgwas)),]$code  
...
if (file.exists(p$filepath.mvLD2)) {
  ...
  #run mvLDSC
  p$mvLD2<-c()
  p$mvLD2$covstruct.mvLDSC<-ldsc.mod(
                                  traits = p$sumstats.sel.lfgwas$mungedpath,
                                  sample.prev =  p$sumstats.sel.lfgwas$samplePrevalence,
                                  population.prev = p$sumstats.sel.lfgwas$populationPrevalence,
                                  trait.names = p$sumstats.sel.lfgwas$lavaan_code,
                                  ld = p$folderpath.data.mvLDSC.ld.1kg,
                                  wld = p$folderpath.data.mvLDSC.ld.1kg,
                                  n.blocks = 600,
                                  info.filter = 0.6,
  ...
  # p$mvLD2$covstruct.mvLDSC$S.orig<-p$mvLD2$covstruct.mvLDSC$S
  # p$mvLD2$covstruct.mvLDSC$S.smooth<-as.matrix((nearPD(p$mvLD2$covstruct.mvLDSC$S, corr = FALSE))$mat)
...
rownames(p$mvLD2$covstruct.mvLDSC$S_Stand.forPlot)<-p$sumstats.sel.lfgwas$name.nice
colnames(p$mvLD2$covstruct.mvLDSC$S_Stand.forPlot)<-p$sumstats.sel.lfgwas$name.nice
...
p$printCorr(
  corr = clipValues(p$mvLD2$covstruct.mvLDSC$S_Stand[p$sumstats.sel$code,p$sumstats.sel$code],-1,1),
  SE = p$mvLD2$covstruct.mvLDSC$S_Stand.SE[p$sumstats.sel$code,p$sumstats.sel$code],
...
p$plots.heritabilities<-paste0(format(diag(p$mvLD2$covstruct.mvLDSC$S),digits=1)," (",format(diag(p$mvLD2$covstruct.mvLDSC$S.SE),digits=1),")")
names(p$plots.heritabilities)<-p$sumstats.sel.lfgwas$code
