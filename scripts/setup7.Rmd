...
# Visualise exported latent factor GWAS results in manhattan and qq plots - UNFINISHED
...
for(iTrait in 1:nrow(chosenSet)){
  ...
  if(nrow(df)>0){
       cplot  <- ggplot(df, aes(x=var, y=val, ymin=low, ymax=high,col=trait,fill=trait)) +
          ...
          geom_hline(yintercept=0, lty=2) +
          geom_point(size=0.6, shape=21, colour="white", stroke = 0.2,position=position_dodge(width = 0.8)) +
          scale_fill_manual(values=c(theme.color$contrastDark2,theme.color$contrastDark3,theme.color$contrastLight3)) +
          scale_color_manual(values=c(theme.color$contrastDark2,theme.color$contrastDark3,theme.color$contrastLight3)) +
          scale_x_discrete(name="Set") +
