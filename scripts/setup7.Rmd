...
for(iSelected in 1:nrow(p$CFA$models.selected)){
  ...
  #test of free model rather than fixed
...
if(p$clOptions$task=="lfgwas" & !file.exists(file.path(p$folderpath.workingDirectory,paste0("lfGWAS.gwas.",p$setup.code,".Rds")))) 
{
  ...
  #p$lfGWAS$sumstats<-head(p$lfGWAS$sumstats, n = 1000)
...
factorCode<-"M20_6_198.COR.ML.F1"
p$sumstats[factorCode,c("code","name","name.nice","n_effective")]<-c(factorCode,factorCode,"Anxiety F1",28196)
factorCode<-"M20_6_198.COR.ML.F2"
p$sumstats[factorCode,c("code","name","name.nice","n_effective")]<-c(factorCode,factorCode,"PTSD F2",17826)
factorCode<-"M20_6_198.COR.ML.F3"
p$sumstats[factorCode,c("code","name","name.nice","n_effective")]<-c(factorCode,factorCode,"Depression F3",12867)
factorCode<-"M20_6_198.COR.ML.F4"
p$sumstats[factorCode,c("code","name","name.nice","n_effective")]<-c(factorCode,factorCode,"Educational attainment F4",386339)
factorCode<-"M20_6_198.COR.ML.F5"
p$sumstats[factorCode,c("code","name","name.nice","n_effective")]<-c(factorCode,factorCode,"Externalising F5",347490)
factorCode<-"M20_6_198.COR.ML.F6"
p$sumstats[factorCode,c("code","name","name.nice","n_effective")]<-c(factorCode,factorCode,"Autism F6",12225)

p$sumstats.sel.code<-c("M20_6_198.COR.ML.F1","M20_6_198.COR.ML.F2","M20_6_198.COR.ML.F3","M20_6_198.COR.ML.F4","M20_6_198.COR.ML.F5","M20_6_198.COR.ML.F6")

##Add sumstat cleaned and munged file paths
p$sumstats[p$sumstats.sel.code,c("cleanedpath")]<-file.path(p$folderpath.data.sumstats.cleaned,paste0(p$sumstats$code,p$filename.suffix.data.sumstats.munged))[which(p$sumstats$code %in% p$sumstats.sel.code)]

p$sumstats[p$sumstats.sel.code,c("mungedpath")]<-file.path(p$folderpath.data.sumstats.munged,paste0(p$sumstats$code,p$filename.suffix.data.sumstats.munged))[which(p$sumstats$code %in% p$sumstats.sel.code)]
...
if(F){ #set to true to re-calculate effective N from file information
  ##Calculate Effective Sample Size for Factor 1 - from the Genomic SEM Wiki
        ...
        effectiveN[iFactor]<-mean(gwasForEffectiveN[,NEF:=((Z_Estimate/est)^2)/(2*MAF*(1-MAF))][,NEF],na.rm=T)
        ...
        #p$lfGWAS$gwas[[cModel$code]][[iFactor]]$NEF<-effectiveN[iFactor]
        
        #add factor trait to sumstats info
        p$sumstats[factorCode,c("code","name","n_effective")]<-c(factorCode,factorCode,effectiveN[iFactor])
}
```
...
p$sumstats.sel.l.code<-c("M20_6_198.COR.ML.F1","M20_6_198.COR.ML.F2","M20_6_198.COR.ML.F3","M20_6_198.COR.ML.F4","M20_6_198.COR.ML.F5","M20_6_198.COR.ML.F6")
p$sumstats.sel.code<-c(p$sumstats.sel.uv.code,p$sumstats.sel.l.code)
p$sumstats.sel<-p$sumstats[which(p$sumstats$code %in% p$sumstats.sel.code),]
...
for(iTrait in 1:length(p$sumstats.sel)){
  ...
  cat("\n",p$lfGWAS$gwas.for.display.info$code,", Genomic inflation factor=", genomicInflationFactorLambdaGC)
  ...
  #p$lfGWAS$gwas.for.display[,Z:=Z/sqrt(genomicInflationFactorLambdaGC)]
   ...
   #if(p$lfGWAS$gwas.for.display.info$lambda_ldsc>0.5)
  ...
  p$lfGWAS$gwas.for.display <- p$lfGWAS$gwas.for.display[P<1e-1]
