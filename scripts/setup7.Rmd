...
chosenSet<-p$sumstats.sel[p$sumstats.sel.set1.code,] #SET SELECTED SUMSTATS HERE
...
for(iTrait in 1:nrow(chosenSet)){
  ...
  #qqman adaptations
  qqmanGwas<-cSumstats
  qqmanGwas$P<- shru::clipValues(qqmanGwas$P,min = 10^(-16), max = NULL)
...
for(igeneset in 1:length(toprocess)){
  ...
  for(iTrait in 1:nrow(chosenSet)){
    #iTrait<-1
    cTraitVal.df <- data.frame(val=cGeneLevelZ[,chosenSet$code[iTrait]],feature=cGeneSetVariables$name, p=cGeneLevelP.FDR[,chosenSet$code[iTrait]])
...
comparisonSet <- p$sumstats.sel[c("NEUR02"),] #comparison traits, apart from general mean
toprocess <- c("gtex8_rnaseq_median","brainspan_rnaseq_mean","msigdb.h.all.v7.5.1","msigdb.c2.cp.biocarta.v7.5.1","msigdb.c2.cp.kegg.v7.5.1","msigdb.c3.mir.mirdb.v7.5.1","msigdb.c3.tft.gtrd.v7.5.1","msigdb.c5.gobp.v7.5.1","msigdb.c5.gocc.v7.5.1","msigdb.c5.gomf.v7.5.1","msigdb.c5.hpo.v7.5.1","msigdb.c5.other.v7.5.1","msigdb.c7.all.v7.5.1","msigdb.c8.all.v7.5.1") #sets to process
...
for(igeneset in 1:length(toprocess)){
  ...
  for(iTrait in 1:nrow(chosenSet)){
    #iTrait<-1
    if(length(glplotsgrid)<1 | length(glplotsgrid[iTrait]) <1) glplotsgrid[iTrait]<-list()
    ...
    for(iComparison in 1:nrow(comparisonSet)){
      ...
      cTraitVal.df <- data.frame(val=cGeneLevelZ[,chosenSet$code[iTrait]]-cGeneLevelZ[,comparisonSet$code[iComparison]],feature=cGeneSetVariables$name, p=cGeneLevelP.FDR[,chosenSet$code[iTrait]])
      #cTraitVal.df <- cTraitVal.df[cTraitVal.df$p<0.1,]
      cTraitVal.df <- head(cTraitVal.df[order(cTraitVal.df$val,decreasing = T),],60)
      cTraitVal.df$sig<-ifelse(cTraitVal.df$p<0.05,"s","n")
      if(nrow(cTraitVal.df)>1 & chosenSet$code[iTrait]!=comparisonSet$code[iComparison]){
        cplot<-ggplot(cTraitVal.df, aes(x = reorder(feature,val), y = val, fill=sig)) + 
        scale_fill_manual(name = "stat. significance",
                       values = c("s" = theme.color$contrastLight3,
                                  "n" = theme.color$contrastDark1),
      ...
      cTraitVal.df1 <- data.frame(
        ...
        val0=cGeneLevelBeta[,chosenSet$code[iTrait]],
      ...
      cTraitVal.df2 <- data.frame(
        ...
        var=cGeneSetVariables$name,
        val0=cGeneLevelBeta[,comparisonSet$code[iComparison]],
      ...
      cTraitVal.df1 <- cTraitVal.df1[(cTraitVal.df1$p<0.05|cTraitVal.df1$p2<0.05) & !is.nan(cTraitVal.df1$se) & cTraitVal.df1$val>0,]
      cTraitVal.df2 <- cTraitVal.df2[cTraitVal.df2$var %in% cTraitVal.df1$var & cTraitVal.df2$val>0,]
      
      df <- rbind(cTraitVal.df1,cTraitVal.df2)
      
      cTraitVal.df1$z <- cTraitVal.df1$val/cTraitVal.df1$se
      dfOrder <- head(na.omit(cTraitVal.df1[order(-cTraitVal.df1$z,na.last = T),]),20)
...
for(iTrait in 1:nrow(chosenSet)){
  ...
  df <- df[which(df$set!="gtex8_rnaseq_median"),]
  ...
  if(nrow(df)>0){
       cplot  <- ggplot(df, aes(x=var, y=val, ymin=low, ymax=high,col=trait,fill=trait)) +
          geom_linerange(size=1,position=position_dodge(width = 0.4)) +
          geom_hline(yintercept=0, lty=2) +
          geom_point(size=1.5, shape=21, colour="white", stroke = 0.5,position=position_dodge(width = 0.5)) +
          scale_fill_manual(values=c(theme.color$contrastLight3,theme.color$contrastDark3)) +
          scale_color_manual(values=c(theme.color$contrastLight3,theme.color$contrastDark3)) +
          scale_x_discrete(name="Set") +
          ...
          theme_bw() +
          ggtitle(label = paste0("Gene-level MAGMA regression betas and standard errors"))

        ggsave(filename=file.path(p$folderpath.plots,chosenSet$code[iTrait], paste0("genelevel.pair.",chosenSet$code[iTrait],".png")), plot = cplot, width = 600, height = 800, units = "mm")
  }
