...
if(p$clOptions$task=="lfgwas" & !file.exists(file.path(p$folderpath.workingDirectory,paste0("lfGWAS.gwas.",p$setup.code,".Rds")))) 
{
  ...
  colAll<-c("SNP","CHR","BP","MAF","A1","A2",colBeta,colSE)
  ...
  #omit NA rows - remove if doing any imputation or replacement instead
  #p$lfGWAS$sumstats<-na.omit(p$lfGWAS$sumstats)
  ...
  for(iChr in 1:22){
    ...
    ldscores[[iChr]]<-suppressMessages(read_delim(
        file.path(p$folderpath.data.mvLDSC.ld.1kg, paste0(iChr, ".l2.ldscore.gz")),
        delim = "\t", escape_double = FALSE, trim_ws = TRUE, progress = FALSE))
    ldscM<-suppressMessages(read_csv(file.path(p$folderpath.data.mvLDSC.ld.1kg, paste0(iChr, ".l2.M_5_50")), col_names = FALSE))
    ldscores[[iChr]]$M<-ldscM[[1]]
  }
  ldscores<-rbindlist(ldscores)
  p$lfGWAS$sumstats[ldscores, on='SNP', c('L2','M') := list(i.L2,i.M)]
  ...
  cCHR<-1
  cSS<-p$lfGWAS$sumstats[CHR==eval(cCHR),.(SNP,MAF,CHR,BP,L2,BETA=get(cBeta),SE=get(cSE),VAR=get(cSE)^2,Z=get(cBeta)/get(cSE))]
  setkeyv(cSS, cols = c("SNP","BP")) #chromosome is fixed per chromosome loop
  #1 cM equates to ~1M bp
  for(i in 1L:1000){
  #for(i in 1L:nrow(cSS)){
    #i<-1L
    cBP<-cSS[i,BP]
    #frame<-cSS[BP!=cBP & BP< cBP+frameLenHalf & BP > cBP-frameLenHalf, .(BETA,SE,VAR,L2,W=L2/(abs(cBP-BP)*VAR))][,.(BETA,SE,VAR,L2,W,WBETA=W*BETA,WSE=W*SE)]
    frame<-cSS[BP!=cBP & BP< cBP+frameLenHalf & BP > cBP-frameLenHalf, .(BETA,SE,VAR,L2,W=L2/VAR)][,.(BETA,SE,VAR,L2,W,WBETA=W*BETA,WSE=W*SE)]
    #frame<-cSS[BP!=cBP & BP< cBP+frameLenHalf & BP > cBP-frameLenHalf, .(BETA,SE,VAR,L2,W=L2)][,.(BETA,SE,VAR,L2,W,WBETA=W*BETA,WSE=W*SE)]
    k<-nrow(frame[!is.na(BETA),])
    W.sum<-sum(frame$W,na.rm = T)
    frame[,IMP.TERM:=WBETA/W.sum][,IMPSE.TERM:=WSE/W.sum]
    if(k>0){
      set(x = cSS,i = i,j = cImp,
          value = sum(frame$IMP.TERM,na.rm = T)
            )
       set(x = cSS,i = i,j = cImpSE,
          value = sum(frame$IMPSE.TERM,na.rm = T)
            )
      set(x = cSS,i = i,j = cImpK,
          value = k
            )
    }
  }
  
  #cSS[,ZIMP:=IMP/IMPSE][,DIFF:=abs(IMP)-abs(BETA)][,DIFFSE:=IMPSE-SE][,ZDIFF:=abs(ZIMP)-abs(Z)][,DIFF2:=DIFF^2][,ZDIFF2:=ZDIFF^2]
  cSS[,ZIMP:=IMP/IMPSE][,DIFF:=IMP-BETA][,DIFFSE:=IMPSE-SE][,ZDIFF:=ZIMP-Z][,DIFF2:=DIFF^2][,ZDIFF2:=ZDIFF^2]
  sqrt(mean(cSS[which(MAF>0.05),]$ZDIFF2,na.rm=T))
  sqrt(mean(cSS[which(MAF<0.05),]$ZDIFF2,na.rm=T))
  
  rmse2 <- sqrt(mean(cSS$ZDIFF2,na.rm=T))
  rmse2
  #rmse<-rmse2 #10000
  
  View(head(cSS, n=1000))
...
p$sumstats.sel.code<-c("PRE.F1","PRE.F2","GSEM.F1","GSEM.F2","GSEM.F3","GSEM.F4","GSEM.F5","GSEM.F6","nmeta.F1","nmeta.F3")
...
p$sumstats.sel.code<-c(p$sumstats.sel.uv.code,p$sumstats.sel.lfgwas.code)
p$sumstats.sel<-p$sumstats[which(p$sumstats$code %in% p$sumstats.sel.code),]
...
colnames(p$mvLD2$covstruct.mvLDSC$I)<-colnames(p$mvLD2$covstruct.mvLDSC$S)
rownames(p$mvLD2$covstruct.mvLDSC$I)<-rownames(p$mvLD2$covstruct.mvLDSC$S)
p$sumstats.sel$lambda_ldsc<-diag(p$mvLD2$covstruct.mvLDSC$I[p$sumstats.sel$code,p$sumstats.sel$code])
