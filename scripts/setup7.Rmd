...
for(iTrait in 1:nrow(chosenSet)){
  ...
  # #export ALL
  fwrite(x = cSumstats, file = file.path(p$folderpath.data.sumstats.export,paste0(p$lfGWAS$gwas.for.display.info$code,".gz")), append = F, quote = F, sep = "\t", nThread=5)
...
p$filepath.results.ga<-file.path(p$folderpath.workingDirectory,paste0("results.ga.",p$setup.code,".Rds"))
...
# Assess genetic association overlap

```{r assess genetic association overlap, fig.width=10, fig.height=6, out.width="1600px", out.height="1000px", eval=FALSE, purl=FALSE}
...
p$sumstats.sel.table<-p$sumstats.sel[,c("name.nice","code","n_effective","h2.liability_mvLDSC","h2.se.liability_mvLDSC","lambda_ldsc","lambda1000")] #full trait list
...
p$sumstats.sel.table<-p$sumstats.sel.table[,c("name.nice","code","n_effective","h2.liability_mvLDSC.table","lambda_ldsc")] #"lambda1000"
