...
project$folderpath.data.sumstats.munged<-normalizePath(file.path(project$folderpath.data,"gwas_sumstats","munged"))
...
project$folderpath.data.mvLDSC.ld <- paste0(project$folderpath.data,"/eur_w_ld_chr.1KG_Phase3/") #LD-scores
project$folderpath.data.mvLDSC.wld <- paste0(project$folderpath.data,"/eur_w_ld_chr.1KG_Phase3/") #Weights, if different from LD-scores
...
project$lfGWAS$info.filter=.5
project$lfGWAS$maf.filter=0.005
...
project$sumstats.sel$effect.logit[which(project$sumstats.sel$code=="DEPR08")]=F #https://www.med.unc.edu/pgc/files/2019/04/PGC_MDD_2018_README_third_180316.pdf # The readme states daner format, but that SE is s.e. of log(OR), while the daner format does not state log scale variables here. Assuming linear scale OR, which is consistent with the data.
...
project$sumstats.sel$se.logit[which(project$sumstats.sel$code=="ANXI03")]=F #Assume linear becasue of Kirstin Purves assuring that both were converted to linear scale.
...
project$sumstats.sel$se.logit[which(project$sumstats.sel$code=="DEPR05")]=F #Assume linear scale because of linear OR
project$sumstats.sel$se.logit[which(project$sumstats.sel$code=="DEPR08")]=F #Assume linear scale because of linear OR
...
if(project$clOptions$task=="munge"){
  ...
  project$sumstats.meta <- supermunge(filePaths = project$munge$filesToUse,
            ...
            trait.names = project$munge$traitNamesToUse,
            path.dir.output = project$folderpath.data.sumstats.munged
              ) 
...
#adding standard errors from the V matrices, unstandardised and standardised - should be fixed in the GSEM method by now. No it is not!
