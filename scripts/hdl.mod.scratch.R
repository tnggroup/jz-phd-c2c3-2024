getwd()
source(normalizePath(paste0("../scripts/","setup1.R")))
source(normalizePath(paste0("scripts/","setup1.R")))
...
#    for(d in j:n.traits){
  ...
  d<-1
      ...
      if(j == d){
        ...
        #for (chr in 1:22) {
          ...
          #for (piece in 1:k) {
            ...
            opt = optim(c(h11.wls[3], h11.wls[1]), llfun, N = N1, Nref = Nref, 
                       lam = lam, bstar = bstar1, M = M, lim = exp(-18), 
                      method = "L-BFGS-B", lower = c(0, 0), upper = c(1,10))
            h11.hdl = opt$par
            logL <- opt$value
      ...
      if (j != d){
        ...
        #cat("TRAIT1:",chi1,("N" %in% colnames(gwas1.df)), min(gwas1.df$N),
        #    "\nTRAIT2:",chi2, ("N" %in% colnames(gwas2.df)), min(gwas2.df$N))
...
#  }
  ...
  #return(
    list(V = V,S = S,I = I,complete=complete)
    #)
