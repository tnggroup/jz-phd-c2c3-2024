...
if(!is.null(mvldscComparison)){
  ...
  p$printCorr(
    ...
    number.digits = 3,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Difference in rg, ",titleTemplate,titleAddition,titleAdditionComparison," [abs(mean)=",round(mean(abs(dS_Stand[testOnlyTraitNameCodes,testOnlyTraitNameCodes])),digits = 2),"]")
  )
  ...
  p$printCorr(
    ...
    number.digits = 1,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Relative (%) difference in rg, ",titleTemplate,titleAddition,titleAdditionComparison," [mean=",round(mean(abs(dS_Stand[testOnlyTraitNameCodes,testOnlyTraitNameCodes])),digits = 1),"]")
  )
  ...
  p$printCorr(
    ...
    number.digits = 4,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Difference in rg S.E., ",titleTemplate,titleAddition,titleAdditionComparison," [mean=",round(mean(dS.S_Stand.SE[testOnlyTraitNameCodes,testOnlyTraitNameCodes]),digits = 4),"]")
  )
  ...
  p$printCorr(
    ...
    number.digits = 1,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Relative (%) difference in rg S.E., ",titleTemplate,titleAddition,titleAdditionComparison," [mean=",round(mean(dS.S_Stand.SE[testOnlyTraitNameCodes,testOnlyTraitNameCodes]),digits = 1),"]")
  )
  ...
  p$printCorr(
    ...
    number.digits = 3,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Difference in rg Coefficient of Variation (CV), ",titleTemplate,titleAddition,titleAdditionComparison," [mean=",round(mean(dCV[testOnlyTraitNameCodes,testOnlyTraitNameCodes]),digits = 3),"]")
  )
  ...
  p$printCorr(
    ...
    number.digits = 3,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("rg dCV significance (FDR corrected, ",iEV," effective tests), ",titleTemplate,titleAddition,titleAdditionComparison)
  )
...
if(p$clOptions$task=="mvLD" || (!file.exists(p$filepath.mvLD) & p$clOptions$task=="0"))
{
  ...
  # cat("\n\n*** LDSC: covstruct.GSEMmvLDSC.hm3 ***\n")
  # p$mvLD$covstruct.GSEMmvLDSC.hm3<-GenomicSEM::ldsc.mod(
  #   traits = p$sumstats.sel.ssimp$mungedpath.ldsc.hm3,
  #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #   trait.names = p$sumstats.sel.ssimp$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.GSEMmvLDSC.1kg ***\n")
  
  # cat("\n\n*** LDSC: covstruct.GSEMmvLDSC.1kg ***\n")
  # p$mvLD$covstruct.GSEMmvLDSC.1kg<-GenomicSEM::ldsc.mod(
  ...
  # cat("\n\n***END LDSC: covstruct.GSEMmvLDSC.1kg ***\n")
  # 
  # cat("\n\n*** LDSC: covstruct.GSEMmvLDSC.hc1kg ***\n")
  # p$mvLD$covstruct.GSEMmvLDSC.hc1kg<-GenomicSEM::ldsc.mod(
  #   traits = p$sumstats.sel.ssimp$mungedpath.ldsc.1kg.orig.maf0_05,
  #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #   trait.names = p$sumstats.sel.ssimp$code,
  #   ld = p$folderpath.data.mvLDSC.ld.hc1kg,
  #   n.blocks = 200,
  #   ldsc.log = p$setup.code.date
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.GSEMmvLDSC.hc1kg ***\n")
  # 
  ...
  # # 
  # # cat("\n\n*** LDSC: covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_05 ***\n")
  # # p$mvLD$covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_05<-ldscpp(
  # #   traits = p$sumstats.sel.ssimp$mungedpath.ldsc.1kg.maf0_05,
  # #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  # #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  # #   trait.names = p$sumstats.sel.ssimp$code,
  # #   ld = p$folderpath.data.mvLDSC.ld.hc1kg,
  ...
  # #   )
  # # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # # cat("\n\n***END LDSC: covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_05 ***\n")
  # # 
  # # cat("\n\n*** LDSC: covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_1 ***\n")
  # # p$mvLD$covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_1<-ldscpp(
  # #   traits = p$sumstats.sel.ssimp$mungedpath.ldsc.1kg.maf0_1,
  # #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  # #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  # #   trait.names = p$sumstats.sel.ssimp$code,
  # #   ld = p$folderpath.data.mvLDSC.ld.hc1kg,
  ...
  # #   )
  # # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # # cat("\n\n***END LDSC: covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_1 ***\n")
  # # 
  # # 
  ...
  # cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw ***\n")
  # p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw<-ldscpp(
  ...
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw ***\n")
  # 
  # cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.nohighld ***\n")
  # p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.nohighld<-ldscpp(
  #   traits = p$sumstats.sel.ssimp$mungedpath,
  #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #   trait.names = p$sumstats.sel.ssimp$code,
  #   filepathLD = p$filepath.SNPReference.hc1kg,
  #   ldsc.log = p$setup.code.date,
  #   filter.region.df = p$highld_b38, #NO HIGH LD REGIONS
  #   preweight.alternativeCorrelationCorrection = T,
  #   preweight.ChiSquare = F,
  #   correctAttenuationBias = F,
  #   doubleRegressionRoutine = F,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw.nohighld ***\n")
  # 
  # 
  # cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw ***\n")
  # p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw<-ldscpp(
  #   traits = p$sumstats.sel.ssimp$mungedpath,
  #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #   trait.names = p$sumstats.sel.ssimp$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw ***\n")
  
  # 
  # cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.jn.varblock.winfo.altcw.maf01test ***\n")
  # p$mvLD$covstruct.mvLDSC.hc1kg.jn.varblock.winfo.altcw.maf01test<-ldscpp(
  #   traits = p$sumstats.sel.ssimp$mungedpath,
  #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #   trait.names = p$sumstats.sel.ssimp$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw.maf01test ***\n")
  # 
  # cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.maf01test ***\n")
  # p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.maf01test<-ldscpp(
  #   traits = p$sumstats.sel.ssimp$mungedpath,
  #   sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #   trait.names = p$sumstats.sel.ssimp$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw.maf01test ***\n")
  # 

  # cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp ***\n")
  #   p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp<-ldscpp(
  #     traits = p$sumstats.sel.ssimp$imputedpath.05cm,
  #     sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #     population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #     trait.names = p$sumstats.sel.ssimp$code,
  #     filepathLD = p$filepath.SNPReference.hc1kg,
  ...
  #     )
  #   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  #   cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw.ldimp ***\n")
  # 
  #   cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.ldimp ***\n")
  #   p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.ldimp.large<-ldscpp(
  #     traits = p$sumstats.sel.ssimp$imputedpath.05cm,
  #     sample.prev =  p$sumstats.sel.ssimp$samplePrevalence.balanced,
  #     population.prev = p$sumstats.sel.ssimp$populationPrevalence,
  #     trait.names = p$sumstats.sel.ssimp$code,
  ...
  #     )
  #   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  #   cat("\n\n***END LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.ldimp ***\n")
  
# cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.large ***\n")
#   p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.large<-ldscpp(
#     traits = p$sumstats.sel$mungedpath,
#     sample.prev =  p$sumstats.sel$samplePrevalence.balanced,
...
#   cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw ***\n")
# 
#   cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.large ***\n")
#   p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.large<-ldscpp(
#     traits = p$sumstats.sel$mungedpath,
#     sample.prev =  p$sumstats.sel$samplePrevalence.balanced,
#     population.prev = p$sumstats.sel$populationPrevalence,
#     trait.names = p$sumstats.sel$code,
...
#     )
#   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
#   cat("\n\n***END LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.large ***\n")
# 
# cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp.large ***\n")
#   p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp.large<-ldscpp(
...
#     )
#   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
#   cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw.ldimp.large ***\n")
# 
#   cat("\n\n*** LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.ldimp.large ***\n")
#   p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.ldimp.large<-ldscpp(
#     traits = p$sumstats.sel$imputedpath.05cm,
#     sample.prev =  p$sumstats.sel$samplePrevalence.balanced,
#     population.prev = p$sumstats.sel$populationPrevalence,
...
#     )
#   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
#   cat("\n\n***END LDSC: covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw.ldimp.large ***\n")
...
# 
# 
#   #temp fix for traits missing NEFF
#   # cSumstats<-data.table::fread(file = "/scratch/users/k19049801/project/JZ_GED_PHD_C1/data/export/gwas.ssimp.cleaned.nohighld/SCHI04.gz", na.strings =c(".",NA,"NA",""), encoding = "UTF-8",check.names = T, fill = T, blank.lines.skip = T, data.table = T, nThread = 6, showProgress = F)
#   # cSumstats[,NEFF:=NEF][,NEF:=NULL] #for ssimp datasets
#   # cSumstats[,VSNP:=2*FRQ*(1-FRQ)]
#   # cSumstats.nNANEFF<-nrow(cSumstats[is.na(NEFF),])
#   # cSumstats[is.na(NEFF),NEFF:=4/(VSNP*(SE^2))]
#   # cSumstats.nNANEFF
#   # cSumstats[,VSNP:=NULL]
#   # data.table::fwrite(x = cSumstats,file = "/scratch/users/k19049801/project/JZ_GED_PHD_C1/data/export/gwas.ssimp.cleaned.nohighld/SCHI04.gz", append = F,quote = F,sep = "\t",col.names = T,nThread=6)
# #
...
#   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
# 
#   # #experimental - there is a bug - do not run!
...
#test
p$sumstats$h2.pd.mvLDSC.1kg.jn.varblock.winfo.altcw<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.jn.varblock.winfo.altcw<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock$S)[p$sumstats.sel.ssimp.code],
  standard_errors2 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock$S.SE)[p$sumstats.sel.ssimp.code],
  n1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  n2 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock$cov.blocks)[p$sumstats.sel.ssimp.code],fullyDependent = T
  )
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.jn.varblock.winfo.altcw")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.jn.varblock.winfo.altcw")]<-test$pTest.standard_errors.adj
#test - compare with LDSC
p$sumstats$h2.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsLDSC<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsLDSC<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
...
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsLDSC")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsLDSC")]<-test$pTest.standard_errors.adj
#test - compare with vbcs
p$sumstats$h2.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsrmethod<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsrmethod<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  n1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  n2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  fullyDependent = T
  )
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsrmethod")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.jn.varblock.winfo.altcw.vsrmethod")]<-test$pTest.standard_errors.adj
...
#test
p$sumstats$h2.pd.mvLDSC.1kg.jn.rblock.winfo.altcw<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.jn.rblock.winfo.altcw<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock$S)[p$sumstats.sel.ssimp.code],
  standard_errors2 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock$S.SE)[p$sumstats.sel.ssimp.code],
  n1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  n2 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock$cov.blocks)[p$sumstats.sel.ssimp.code],fullyDependent = T
  )
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.jn.rblock.winfo.altcw")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.jn.rblock.winfo.altcw")]<-test$pTest.standard_errors.adj
#test - compare with LDSC
p$sumstats$h2.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsLDSC<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsLDSC<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = p$sumstats$h2.LDSC.1kg.maf0_01[p$sumstats$code %in% p$sumstats.sel.ssimp.code],
...
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsLDSC")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsLDSC")]<-test$pTest.standard_errors.adj
#test - - compare with vbcs
p$sumstats$h2.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsrmethod<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsrmethod<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  n1 = diag(p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  n2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  fullyDependent = T
  )
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsrmethod")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.jn.rblock.winfo.altcw.vsrmethod")]<-test$pTest.standard_errors.adj
...
#test
p$sumstats$h2.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw<-NA_real_
test<-test.meta.deltacovariance(
  ...
  values2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock$S)[p$sumstats.sel.ssimp.code],
  standard_errors2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock$S.SE)[p$sumstats.sel.ssimp.code],
  n1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  n2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock$cov.blocks)[p$sumstats.sel.ssimp.code],fullyDependent = T
  )
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw")]<-test$pTest.standard_errors.adj
#test - compare with LDSC
p$sumstats$h2.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw.vsLDSC<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw.vsLDSC<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = p$sumstats$h2.LDSC.1kg.maf0_01[p$sumstats$code %in% p$sumstats.sel.ssimp.code],
...
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw.vsLDSC")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.vbcs.varblock.winfo.altcw.vsLDSC")]<-test$pTest.standard_errors.adj
...
#test
p$sumstats$h2.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock$S)[p$sumstats.sel.ssimp.code],
  standard_errors2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock$S.SE)[p$sumstats.sel.ssimp.code],
  n1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$cov.blocks)[p$sumstats.sel.ssimp.code],
  n2 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock$cov.blocks)[p$sumstats.sel.ssimp.code],fullyDependent = T
  )
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw")]<-test$pTest.standard_errors.adj
#test - compare with LDSC
p$sumstats$h2.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw.vsLDSC<-NA_real_
p$sumstats$h2.se.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw.vsLDSC<-NA_real_
test<-test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$S)[p$sumstats.sel.ssimp.code],
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw$S.SE)[p$sumstats.sel.ssimp.code],
  values2 = p$sumstats$h2.LDSC.1kg.maf0_01[p$sumstats$code %in% p$sumstats.sel.ssimp.code],
...
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw.vsLDSC")]<-test$pTest.values.adj
p$sumstats[p$sumstats.sel.ssimp.code,c("h2.se.pd.mvLDSC.1kg.vbcs.rblock.winfo.altcw.vsLDSC")]<-test$pTest.standard_errors.adj
...
#rblock - only 1kg-based panels

p$cv_coverage.ldsc.hm3.gsem <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.GSEMemulation.hm3, labelString = "hm3 - gsem emulation", groupString = "hm3.gsem")

p$cv_coverage.ldsc.hm3.vbcs.varblock.winfo.altcw <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.hm3.vbcs.varblock.winfo.altcw, labelString = "hm3.vbcs.varblock.winfo.altcw", groupString = "hm3 - varblock")

p$cv_coverage.ldsc.1kg.gsem <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01, labelString = "1kg.gsem", groupString = "1kg - gsem emulation")

p$cv_coverage.ldsc.1kg.vbcs.varblock.winfo.altcw <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw, labelString = "1kg.vbcs.varblock.winfo.altcw", groupString = "1kg - varblock")

# p$cv_coverage.ldsc.hc1kg.vbcs.varblock.winfo.altcw <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw, labelString = "hc1kg.vbcs.varblock.winfo.altcw", groupString = "hc1kg - varblock")

p$cv_coverage.ldsc.1kg.vbcs.rblock.winfo.altcw <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw, labelString = "1kg.vbcs.rblock.winfo.altcw", groupString = "1kg - rblock")

# p$cv_coverage.ldsc.hc1kg.vbcs.rblock.winfo.altcw <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw, labelString = "hc1kg.vbcs.rblock.winfo.altcw", groupString = "hc1kg - rblock")
# 
# p$cv_coverage.ldsc.GSEMemulation.hc1kg.vbcs.rblock.winfo.altcw <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.GSEMemulation.hc1kg.maf0_05, labelString = "ldsc.emulated", groupString = "hc1kg - 200 blocks")
...
p$cv_coverage.ldsc.1kg.gsem,
p$cv_coverage.ldsc.1kg.vbcs.varblock.winfo.altcw,
#p$cv_coverage.ldsc.hc1kg.vbcs.varblock.winfo.altcw,
p$cv_coverage.ldsc.1kg.vbcs.rblock.winfo.altcw
#p$cv_coverage.ldsc.hc1kg.vbcs.rblock.winfo.altcw,
#p$cv_coverage.ldsc.GSEMemulation.hc1kg.vbcs.rblock.winfo.altcw
  )
...
ggplot(data = p$cv_coverage, aes(x=m, y=cv, shape=l, color=g)) +
  ...
  stat_cor(method="pearson", label.x.npc = 0.3,label.y.npc = 0.9, aes(group=g))
  #stat_cor(method="pearson", label.x.npc = "middle",label.y.npc = "bottom")
ggsave(filename=file.path(p$folderpath.plots,"ldsc_m_cv.panel.covG.png"), plot = last_plot(), width = 200, height = 200, units = "mm")
...
#   stat_cor(method="pearson", label.x.npc = 0.5,label.y.npc = 0.4, aes(group=g, color=g))
#   #stat_cor(method="pearson", label.x.npc = "middle",label.y.npc = "bottom")
# ggsave(filename=file.path(p$folderpath.plots,"ldsc_m_se.panel.covG.png"), plot = last_plot(), width = 200, height = 200, units = "mm")
...
ggplot(data = p$cv_coverage[p$cv_coverage$isDiag==T,], aes(x=m, y=cv, shape=l, color=g)) +
  ...
  stat_cor(method="pearson", label.x.npc = 0.4,label.y.npc = 0.8, aes(group=g))
  #stat_cor(method="pearson", label.x.npc = "middle",label.y.npc = "bottom")
ggsave(filename=file.path(p$folderpath.plots,"ldsc_m_cv.panel.h2.png"), plot = last_plot(), width = 200, height = 200, units = "mm")
...
#   stat_cor(method="pearson", label.x.npc = 0.5,label.y.npc = 0.4, aes(group=g, color=g))
#   #stat_cor(method="pearson", label.x.npc = "middle",label.y.npc = "bottom")
# ggsave(filename=file.path(p$folderpath.plots,"ldsc_m_se.panel.h2.png"), plot = last_plot(), width = 200, height = 200, units = "mm")
...
corrForPlot <- clipValues(p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw$S_Stand,-1,1)
pForPlot <- p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw$cov.p
...
par(xpd=TRUE) #keep labels inside margins
pal<-colorRampPalette(c("#FF6666","#EEEEEE","#6666FF"))
corrplot(corr = corrForPlot, method = "circle", insig='blank', order = "original", p.mat = pForPlot, sig.level = c(0.05), pch.cex = 1.5, tl.cex = 0.8, full_col=FALSE, na.label = "square", na.label.col = "grey30", title = "A: LDSC++ genetic correlations, HC1kGP3, number-of-variants defined blocks", tl.pos = 'd', mar=c(0,0,4,0), type = "upper", cl.pos = "r", col = pal(200))$corrPos -> p1 #type = "lower", diag=F, addCoef.col = 'black'
#text(p1$x, p1$y, round(p1$corr, 2))
p1$corr <- ifelse(0==-p1$y + ncol(corrForPlot)-p1$x + 1, NA_real_, p1$corr)
text(p1$x, p1$y, round(p1$corr, 2))
dev.off()

corrForPlot <- clipValues(p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw$S_Stand,-1,1)
pForPlot <- p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw$cov.p
colnames(corrForPlot)<-p$sumstats.sel.ssimp$code.nice
rownames(corrForPlot)<-p$sumstats.sel.ssimp$code.nice
colnames(pForPlot)<-p$sumstats.sel.ssimp$code.nice
rownames(pForPlot)<-p$sumstats.sel.ssimp$code.nice
...
par(xpd=TRUE) #keep labels inside margins
pal<-colorRampPalette(c("#FF6666","#EEEEEE","#6666FF"))
corrplot(corr = corrForPlot, method = "circle", insig='blank', order = "original", p.mat = pForPlot, sig.level = c(0.05), pch.cex = 1.5, tl.cex = 0.8, full_col=FALSE, na.label = "square", na.label.col = "grey30", title = "B: LDSC++ genetic correlations, HC1kGP3, recombination distance defined blocks", tl.pos = 'd', mar=c(0,0,4,0), type = "upper", cl.pos = "r", col = pal(200))$corrPos -> p1 #type = "lower", diag=F, addCoef.col = 'black'
#text(p1$x, p1$y, round(p1$corr, 2))
p1$corr <- ifelse(0==-p1$y + ncol(corrForPlot)-p1$x + 1, NA_real_, p1$corr)
text(p1$x, p1$y, round(p1$corr, 2))
dev.off()
...
# 
# corrForPlot <- clipValues(p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp$S_Stand,-1,1)
# pForPlot <- p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp$cov.p
# colnames(corrForPlot)<-p$sumstats.sel.ssimp$code.nice
# rownames(corrForPlot)<-p$sumstats.sel.ssimp$code.nice
# colnames(pForPlot)<-p$sumstats.sel.ssimp$code.nice
# rownames(pForPlot)<-p$sumstats.sel.ssimp$code.nice
...
# par(xpd=TRUE) #keep labels inside margins
# pal<-colorRampPalette(c("#FF6666","#EEEEEE","#6666FF"))
# corrplot(corr = corrForPlot, method = "circle", insig='blank', order = "original", p.mat = pForPlot, sig.level = c(0.05), pch.cex = 1.5, tl.cex = 0.8, full_col=FALSE, na.label = "square", na.label.col = "grey30", title = "B: LDSC++ genetic correlations, HC1kGP3 + LD-IMP reference:\nLD-IMP augmented GWAS + GWIS of male BMI (LD-IMP augmented height & weight)", tl.pos = 'd', mar=c(0,0,4,0), type = "upper", cl.pos = "r", col = pal(200))$corrPos -> p1 #type = "lower", diag=F, addCoef.col = 'black'
# #text(p1$x, p1$y, round(p1$corr, 2))
# p1$corr <- ifelse(0==-p1$y + ncol(corrForPlot)-p1$x + 1, NA_real_, p1$corr)
# text(p1$x, p1$y, round(p1$corr, 2))
# dev.off()
...
plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.GSEMemulation.hm3, folderpath.plots = p$folderpath.plots, code = "original.hm3", titleTemplate = "original LDSC (Genomic SEM emulated), HM3 reference", titleAddition = "\nBlock Jackknife Resampling")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01, folderpath.plots = p$folderpath.plots, code = "original.1kg", titleTemplate = "original LDSC (Genomic SEM emulated), 1kGP3 reference", titleAddition = "\nBlock Jackknife Resampling")

#explicit test of LDSC++ GSEMemulation vs original LDSC
test.GSEMemulation_vs_originalLDSC<-shru::test.meta.deltacovariance(
  values1 = diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S),
  standard_errors1 = diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE),
  n1 = diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$cov.blocks),
  values2 = p$sumstats[p$sumstats$code %in% colnames(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S) ,c("h2.LDSC.1kg.maf0_01")],
  standard_errors2 = p$sumstats[p$sumstats$code %in% colnames(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S),c("h2.se.LDSC.1kg.maf0_01")],
  n2 = rep(200,nrow(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S)),
...
test.GSEMemulation_vs_originalLDSC$pTest.values.adj

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.hm3res, folderpath.plots = p$folderpath.plots, code = "original.1kg.hm3res", titleTemplate = "Original LDSC (Genomic SEM emulated), 1kGP3 reference restricted to HM3 variants", titleAddition = "\nBlock Jackknife Resampling")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01,titleAdditionComparison = "\ncomparing with original LDSC (Genomic SEM emulated)")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.r", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01,titleAdditionComparison = "\ncomparing with original LDSC (Genomic SEM emulated)")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs_jn", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw,titleAdditionComparison = "\ncomparing with Block Jackknife Resampling")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.r_jn", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw,titleAdditionComparison = "\ncomparing with Block Jackknife Resampling")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.v_r", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw,titleAdditionComparison = "\ncomparing with recombination distance blocks")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.jn.v_r", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw,titleAdditionComparison = "\ncomparing with recombination distance blocks")

#weighting extensions comparisons

#full
plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.v.winfo.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.r.winfo.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.jn.v.winfo.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.jn.r.winfo.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

#winfo only
plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.v.winfo", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.r.winfo", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo, folderpath.plots = p$folderpath.plots, code = "1kg.jn.v.winfo", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo, folderpath.plots = p$folderpath.plots, code = "1kg.jn.r.winfo", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

#altcw only
plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.v.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.vbcs.r.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.jn.v.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw, folderpath.plots = p$folderpath.plots, code = "1kg.jn.r.altcw", titleTemplate = "LDSC++, 1kGP3 reference", titleAddition = "\nblock jackknife resampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock,titleAdditionComparison = "\ncomparing with not applying weighting extensions")

# plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "hc1kg.vbcs.varblock", titleTemplate = "LDSC++, HC1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, number-of-variants defined blocks", mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.altcw,titleAdditionComparison = "\ncomparing with 1kGP3 reference") 
# 
# plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.rblock.winfo.altcw, folderpath.plots = p$folderpath.plots, code = "hc1kg.vbcs.rblock", titleTemplate = "LDSC++, HC1kGP3 reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance defined blocks", mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.altcw,titleAdditionComparison = "\ncomparing with 1kGP3 reference") 

# plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw.ldimp, folderpath.plots = p$folderpath.plots, code = "hc1kg.ldimp.vbcs", titleTemplate = "LDSC++, HC1kGP3 reference, LD-IMP", titleAddition = "\nVariable Block-Count Sampling",mvldscComparison = p$mvLD$covstruct.mvLDSC.hc1kg.vbcs.varblock.winfo.altcw,titleAdditionComparison = "\ncomparing with HC1kGP3 reference")
