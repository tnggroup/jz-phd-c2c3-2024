...
p$folderpath.data.sumstats.munged<-normalizePath(file.path(p$folderpath.data,"gwas_sumstats","munged_1kg_eur_supermunge_filtered"))
...
#via 10.1101/211821
p$highld<-data.frame(matrix(data=NA,ncol=0,nrow=0))

p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(1,48287980,52287979)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(2,86088342,101041482)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(2,134666268,138166268)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(2,183174494,190174494)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(3,47524996,50024996)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(3,83417310,96017310)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(5,44464243,50464243)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(5,97972100,100472101)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(5,128972101,131972101)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(5,135472101,138472101)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(6,25392021,33392022)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(6,56892041,63942041)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(6,139958307,142458307)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(7,55225791,66555850)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(8,7962590,11962591)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(8,42880843,49837447)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(8,111930824,114930824)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(10,36959994,43679994)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(11,46043424,57243424)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(11,87860352,90860352)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(12,33108733,41713733)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(12,111037280,113537280)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(17,31799963,33389579)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(17,40928985,42139672)
p$highld[(nrow(p$highld)+1),c("CHR","BP1","BP2")]<-c(20,32536339,35066586)

setDT(p$highld)
setkeyv(p$highld, cols = c("CHR","BP1","BP2"))

# #test filter
# test<-readFile(filePath = p$sumstats.sel[1,]$mungedpath)
# 
# nrow(test)
# 
# setDT(test)
# setkeyv(test, cols = c("CHR","BP"))
# #test2<-p$highld[test,on = .(CHR==CHR, BP <= BP1, BP >= BP2)]
# #test2<-merge(p$highld,)
# 
# test2<-test
# 
# #NOT DONE!!!
...
#   #isegment<-1
#   test2 <- test2[!(CHR==p$highld$CHR[isegment] & BP>=p$highld$BP1[isegment] & BP<=p$highld$BP2[isegment]),]
#   #testExc <- test2[(CHR==1 & BP>=48287980 & BP<=52287979),]
#   #test2 <- test2[!(CHR==1 & BP>=48287980 & BP<=52287979),]
# }
# 
# nrow(test2)
...
for(iSelected in 1:nrow(p$CFA$models.selected)){
  ...
  scale_fill_gradient2(name = "Loading", 
                       ...
                       midpoint=0, guide=F) +
  ylab("Std. factor loading") + #improve y-axis label
...
factorCode<-"GSEM.F1"
p$sumstats[factorCode,c("code","code.file","name","n_effective")]<-c(factorCode,"M20_6_172.COR.ML.F1","fNeuroticism",364979)
factorCode<-"GSEM.F2"
p$sumstats[factorCode,c("code","code.file","name","n_effective")]<-c(factorCode,"M20_6_172.COR.ML.F2","fPsychoticism",159862)
factorCode<-"GSEM.F3"
p$sumstats[factorCode,c("code","code.file","name","n_effective")]<-c(factorCode,"M20_6_172.COR.ML.F3","fDepression",448644)
factorCode<-"GSEM.F4"
p$sumstats[factorCode,c("code","code.file","name","n_effective")]<-c(factorCode,"M20_6_172.COR.ML.F4","fRisktaking",46293)
factorCode<-"GSEM.F5"
p$sumstats[factorCode,c("code","code.file","name","n_effective")]<-c(factorCode,"M20_6_172.COR.ML.F5","fDeprivation",271911)
factorCode<-"GSEM.F6"
p$sumstats[factorCode,c("code","code.file","name","n_effective")]<-c(factorCode,"M20_6_172.COR.ML.F6","fNeuropsyche",91409)
