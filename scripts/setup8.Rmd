...
p$sumstats$name.nice[which(p$sumstats$code=="DEPR09")]="MDD"
...
#p$sumstats.sel.ssimp.code <- c("ANXI03","DEPR09","DEPR05","NEUR02","BODY11")
#full
...
if(p$clOptions$task=="impute"
  ...
  p$munge$results <- supermunge(
            ...
            traitNames = p$munge$traitNamesToUse,
            imputeFromLD=T,
            imputeFrameLenBp = 10000, #TEMPORARY to get the 10k imputation
            imputeFrameLenCM=NULL,
            region.imputation.filter_df=p$highld_b38, #provide the high-ld regions to not use for imputation
            pathDirOutput = p$folderpath.data.sumstats.imputed.10 #TEMPORARY to get the 10k imputation
              )
}
...
if(p$clOptions$task=="munge" ||  p$clOptions$task=="impute" || p$clOptions$task=="munge2") quit(save = "no")
...
if(p$clOptions$task=="merge"){
  ...
  for(cTraitCode in p$sumstats.sel.ssimp.code) {
    ...
    if(length(fullTraitList)>0){
      ...
      shru::supermunge(list_df = list(fullTrait) , traitNames = cTraitCode, process = F, nThreads = 5, pathDirOutput = p$folderpath.data.sumstats.imputed.ssimp)
...
if(p$clOptions$task=="mvLD" || (!file.exists(p$filepath.mvLD) & p$clOptions$task=="0"))
{
  ...
  # 
  # p$mvLD$covstruct.mvLDSC.1kg.noimp<-ldsc.mod(
  #   traits = p$sumstats.sel$mungedpath,
  #   sample.prev =  p$sumstats.sel$samplePrevalence,
  #   population.prev = p$sumstats.sel$populationPrevalence,
  #   trait.names = p$sumstats.sel$code,
  ...
  # p$mvLD$covstruct.mvLDSC.1kg.10<-ldsc.mod(
  #   traits = p$sumstats.sel$imputedpath.10,
  #   sample.prev =  p$sumstats.sel$samplePrevalence,
  #   population.prev = p$sumstats.sel$populationPrevalence,
  #   trait.names = p$sumstats.sel$code,
  #   ld = p$folderpath.data.mvLDSC.ld.1kg,
  ...
  # p$mvLD$covstruct.mvLDSC.1kg.500<-ldsc.mod(
  ...
  # p$mvLD$covstruct.mvLDSC.1kg.500.2<-ldsc.mod(
  #   traits = p$sumstats.sel$imputedpath.500,
  #   sample.prev =  p$sumstats.sel$samplePrevalence,
  #   population.prev = p$sumstats.sel$populationPrevalence,
  #   trait.names = p$sumstats.sel$code,
  ...
  # p$mvLD$covstruct.mvLDSC.1kg.ssimp<-ldsc.mod(
  #   traits = p$sumstats.sel$imputedpath.ssimp,
  #   sample.prev =  p$sumstats.sel$samplePrevalence,
  #   population.prev = p$sumstats.sel$populationPrevalence,
  #   trait.names = p$sumstats.sel$code,
  #   ld = p$folderpath.data.mvLDSC.ld.1kg,
  ...
  for(cTrait in p$sumstats.sel$code){
    p$mvLD$h2.500[cTrait] <- list(
      ldsc.orig(trait_codes = list(cTrait),trait_filepaths = p$sumstats.sel[cTrait,]$imputedpath.500, ldsc_filepath = "/users/k19049801/project/JZ_GED_PHD_ADMIN_GENERAL/software/ldsc/ldsc.py", ld_filepath = file.path(p$folderpath.data.mvLDSC.ld.1kg,"hc1kgp3.b38.eur.1cm.400blocks"),samp_prev = p$sumstats.sel$samplePrevalence[cTrait], pop_prev = p$sumstats.sel$populationPrevalence[cTrait])
      )
  }
  ...
  for(cTrait in p$sumstats.sel$code){
    p$mvLD$h2.ssimp[cTrait] <- list(
      ldsc.orig(trait_codes = list(cTrait),trait_filepaths = p$sumstats.sel[cTrait,]$imputedpath.ssimp, ldsc_filepath = "/users/k19049801/project/JZ_GED_PHD_ADMIN_GENERAL/software/ldsc/ldsc.py", ld_filepath = file.path(p$folderpath.data.mvLDSC.ld.1kg,"hc1kgp3.b38.eur.1cm.400blocks"),samp_prev = p$sumstats.sel$samplePrevalence[cTrait], pop_prev = p$sumstats.sel$populationPrevalence[cTrait])
      )
  }
...
#heatmaply()
heatmaply(
    clipValues(p$mvLD$covstruct.mvLDSC$S_Stand,-1,1),
    cellnote = round(clipValues(p$mvLD$covstruct.mvLDSC$S_Stand,-1,1),digits = 2),
    cellnote_size = 9,
    ...
    file=file.path(p$folderpath.plots,paste0("rg_cluster.indicator.png")),
...
p$printCorr(
  corr = 100* (p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE)/median(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE),
  SE = NULL,
...
p$printCorr(
  corr = 100*(p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE)/median(p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE),
  SE = NULL,
...
p$sumstats.sel.table<-p$sumstats.sel[,c("name.nice","code","reference.nice","year","n_case_total","n_control", "samplePrevalence","populationPrevalence","h2.liability_mvLDSC.hm3","h2.se.liability_mvLDSC.hm3","h2.liability_mvLDSC.1kg.noimp","h2.se.liability_mvLDSC.1kg.noimp","h2.liability_mvLDSC.1kg.ldimp","h2.se.liability_mvLDSC.1kg.ldimp","h2.liability_mvLDSC.1kg.ssimp","h2.se.liability_mvLDSC.1kg.ssimp")]
...
p$sumstats.sel.table<-p$sumstats.sel.table[,c("name.nice","code","reference.nice","year","n_case_total","n_control", "samplePrevalence","populationPrevalence","h2.liability_mvLDSC.hm3.table","h2.liability_mvLDSC.1kg.noimp.table","h2.liability_mvLDSC.1kg.ssimp.table","h2.liability_mvLDSC.1kg.ldimp.table")]
...
p$plots.sumstats.sel.table<-p$sumstats.sel.table %>% 
  ...
  tab_header(
    ...
    h2.liability_mvLDSC.hm3.table = html("h<sup>2</sup><sub>hm3</sub> (S.E.)"),
    h2.liability_mvLDSC.1kg.noimp.table = html("h<sup>2</sup><sub>1kg</sub> (S.E.)"),
    h2.liability_mvLDSC.1kg.ldimp.table = html("h<sup>2</sup><sub>1kg & LD-IMP</sub> (S.E.)"),
    h2.liability_mvLDSC.1kg.ssimp.table = html("h<sup>2</sup><sub>1kg & SSIMP</sub> (S.E.)")
  ) %>%
  tab_style(
