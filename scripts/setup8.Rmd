...
#p$sumstats.sel.ssimp.code <- c("BIPO02","DEPR05") #TEST
#full
...
#p$sumstats.sel.code<-c("ADHD05","ALCD03","ANOR02","ANXI03","ANXI04","AUTI07","BODY11","BIPO02", "BVOL01","DEPR05","DEPR09","EDUC03","HEAL01","INCO03","INSO03","NEUR02", "PTSD04","RISK02","RISK03","SCHI04","SUBJ01","TIRE01")
...
if(p$clOptions$task=="impute"
  ...
  p$munge$results <- supermunge(
            ...
            region.imputation.filter_df=p$highld_b38, #provide the high-ld regions to not use for imputation
...
if(p$clOptions$task=="merge"){
  ...
  for(cTraitCode in p$sumstats.sel.ssimp.code) {
    ...
    if(length(fullTraitList)>0){
      ...
      mergedFullTrait <- merge(fullTrait,mungedTrait, by=c("SNP"), all = T)
      mergedFullTrait[(is.na(Z.x)&!is.na(Z.y)) || P.x>P.y,c("A1.x","A2.x","Z.x","P.x","N.x", "LD_IMP"):=list(A1.y,A2.y,Z.y,P.y,N.y,1.0)]
      mergedFullTrait[is.na(chr),chr:=CHR][is.na(pos),pos:=BP][is.na(maf),maf:=FRQ]
      
      mergedFullTrait<-mergedFullTrait[,.(SNP,A1=A1.x,A2=A2.x,CHR=chr,BP=pos,FRQ=maf,Z=Z.x,N=N.x,P=P.x,LD_IMP)]
      
      colnames(mergedFullTrait)
...
if(p$clOptions$task=="mvLD" || (!file.exists(p$filepath.mvLD) & p$clOptions$task=="0"))
{
  ...
  #test of ldsc2
  # p$mvLD$covstruct.mvLDSC.1kg.ldsc2<-ldsc2(
  ...
  #   )
  # 
  # p$mvLD$covstruct.mvLDSC.1kg.ldscmod<-ldsc.mod(
  #   traits = p$sumstats.sel$mungedpath,
  #   sample.prev =  p$sumstats.sel$samplePrevalence,
  #   population.prev = p$sumstats.sel$populationPrevalence,
  #   trait.names = p$sumstats.sel$code,
  ...
  p$mvLD$covstruct.mvLDSC.hm3<-ldsc2(
    ...
    traits = p$sumstats.sel$mungedpath2,
    sample.prev =  p$sumstats.sel$samplePrevalence,
    population.prev = p$sumstats.sel$populationPrevalence,
    trait.names = p$sumstats.sel$code,
  ...
  # p$mvLD$covstruct.mvLDSC.1kg.noimp.play<-ldsc.play(
  #   traits = p$sumstats.sel$mungedpath2,
  #   sample.prev =  p$sumstats.sel$samplePrevalence,
  #   population.prev = p$sumstats.sel$populationPrevalence,
  #   trait.names = p$sumstats.sel$code,
  ...
  p$mvLD$covstruct.mvLDSC.1kg.noimp<-ldsc2(
    traits = p$sumstats.sel$mungedpath2,
    sample.prev =  p$sumstats.sel$samplePrevalence,
    population.prev = p$sumstats.sel$populationPrevalence,
    trait.names = p$sumstats.sel$code,
  ...
  p$mvLD$covstruct.mvLDSC.1kg.500<-ldsc2(
    traits = p$sumstats.sel$imputedpath2.500,
    sample.prev =  p$sumstats.sel$samplePrevalence,
    population.prev = p$sumstats.sel$populationPrevalence,
    trait.names = p$sumstats.sel$code,
    filepathLD = p$filepath.SNPReference.1kg,
    ...
    maf.filter = 0.01,
    #chisq.min = 0.1,
    #ldimp.adjustN = T, #temporary!!! #temporary!!! #This should be fixed in the currently exported versions - NOT in the ordinary munged versions though!
    region.filter_df = p$highld_b38,
    #mhc.filter = 37,
    N = p$sumstats.sel$n_total,
    ldsc.log = p$setup.code.date
  ...
  p$mvLD$covstruct.mvLDSC.1kg.ssimp<-ldsc2(
...
p$sumstats.sel$h2.liability_mvLDSC.1kg.noimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.noimp$S[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.noimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE[p$sumstats.sel$code,p$sumstats.sel$code])
...
p$sumstats.sel$h2.liability_mvLDSC.1kg.ssimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.ssimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE[p$sumstats.sel$code,p$sumstats.sel$code])

p$sumstats.sel$h2_orig.liability_mvLDSC.1kg.noimp<-as.numeric(lapply(X = p$mvLD$h2.munged, FUN = function(x){ifelse(any(colnames(x$df)=="h2_liab"),x$df$h2_liab,x$df$h2_obs)}))
p$sumstats.sel$h2_orig.se.liability_mvLDSC.1kg.noimp<-as.numeric(lapply(X = p$mvLD$h2.munged, FUN = function(x){ifelse(any(colnames(x$df)=="h2_liab_se"),x$df$h2_liab_se,x$df$h2_obs_se)}))
...
# 
#deltapanel
p$mvLD$covstruct.mvLDSC$S_Stand.deltapanel <- 100*(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand - p$mvLD$covstruct.mvLDSC.hm3$S_Stand)/abs(p$mvLD$covstruct.mvLDSC.hm3$S_Stand)
p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltapanel <- 100*(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE - p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE)/abs(p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE)

#deltaimp 500
p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp.500 <- 100*(p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand)/abs(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand)
p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp.500 <- 100*(p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand.SE - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand.SE)/abs(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand.SE)

#deltaimpnoimp 500
p$mvLD$covstruct.mvLDSC$S_Stand.deltaimpnoimp.500 <- 100*(p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand - p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand)/abs(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand)
p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimpnoimp.500 <- 100*(p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE)/abs(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE)

#deltaimpnoimp ssimp
p$mvLD$covstruct.mvLDSC$S_Stand.deltaimpnoimp.ssimp <- 100*(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand - p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand)/abs(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand)
p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimpnoimp.ssimp <- 100*(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE)/abs(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE)
...
#heatmaply()
heatmaply(
    clipValues(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand,-1,1),
    cellnote = round(clipValues(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand,-1,1),digits = 2),
    cellnote_size = 9,
...
heatmaply(
    clipValues(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand,-1,1),
    cellnote = round(clipValues(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand,-1,1),digits = 2),
    cellnote_size = 9,
    cellnote_textposition = "middle center",
    show_dendrogram = c(T,F),
    xlab = "Trait GWAS",
...
p$printCorr(
  corr = clipValues(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand,-1,1),
  SE = p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE,
...
p$printCorr(
  corr = clipValues(p$mvLD$covstruct.mvLDSC$S_Stand,-1,1),
  SE = p$mvLD$covstruct.mvLDSC$S_Stand.SE,
...
p$printCorr(
  corr = clipValues(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand,-1,1),
  SE = p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand.SE,
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand - p$mvLD$covstruct.mvLDSC.hm3$S_Stand,
  SE = NULL,
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE - p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE,
  SE = NULL,
...
p$printCorr(
  corr = 100*(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE - p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE)/median(p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE),
  SE = NULL,
...
p$printCorr(
  corr = 100*(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE/median(p$mvLD$covstruct.mvLDSC.1kg.noimp$S_Stand.SE) - p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE/median(p$mvLD$covstruct.mvLDSC.hm3$S_Stand.SE)),
  SE = NULL,
  filename = file.path(p$folderpath.plots,"rgse.corr.indicator.deltapanel.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 0
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.noimp$S - p$mvLD$covstruct.mvLDSC.hm3$S,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covg.indicator.deltapanel.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 2
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE - p$mvLD$covstruct.mvLDSC.hm3$S.SE,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.indicator.deltapanel.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 3
...
p$printCorr(
  corr = 100*(p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE - p$mvLD$covstruct.mvLDSC.hm3$S.SE)/median(p$mvLD$covstruct.mvLDSC.hm3$S.SE),
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.rel.indicator.deltapanel.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 0
...
p$printCorr(
  corr = 100*(p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE/median(p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE) - p$mvLD$covstruct.mvLDSC.hm3$S.SE/median(p$mvLD$covstruct.mvLDSC.hm3$S.SE)),
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.corr.indicator.deltapanel.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 0
...
# p$printCorr(
#   corr = p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand,
#   SE = NULL,
...
# 
# p$printCorr(
#   corr = p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand.SE - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S_Stand.SE,
#   SE = NULL,
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.500$S - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covg.indicator.deltaimp.500.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 2
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.indicator.deltaimp.500.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 3
...
p$printCorr(
  corr = 100* (p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE)/p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.rel.indicator.deltaimp.500.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 0
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.500$S - p$mvLD$covstruct.mvLDSC.1kg.noimp$S,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covg.indicator.deltaimpnoimp.500.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 2
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.indicator.deltaimpnoimp.500.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 3
...
p$printCorr(
  corr = 100*(p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE)/p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.rel.indicator.deltaimpnoimp.500.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 0
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.ssimp$S - p$mvLD$covstruct.mvLDSC.1kg.noimp$S,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covg.indicator.deltaimpnoimp.ssimp.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 2
...
p$printCorr(
  corr = p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.indicator.deltaimpnoimp.ssimp.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 3
...
p$printCorr(
  corr = 100 * (p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE - p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE)/p$mvLD$covstruct.mvLDSC.1kg.noimp$S.SE ,
  SE = NULL,
  filename = file.path(p$folderpath.plots,"covgse.rel.indicator.deltaimpnoimp.ssimp.png"),
  addrect =NULL,
  is.corr = F,
  number.cex = 1.1,
  number.digits = 0
...
cat("\nLDSC stats - LD-IMP")
print(p$mvLD$covstruct.mvLDSC.1kg$impstats)

cat("\nLDSC stats - SSIMP")
print(p$mvLD$covstruct.mvLDSC.1kg.ssimp$impstats)

cat("\nDifference statistics")
cat("\ndeltapanel\n")
mean(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltapanel))
median(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltapanel))
max(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltapanel))
min(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltapanel))

cat("\ndeltapanel (S.E.)\n")
mean(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltapanel)
median(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltapanel)
max(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltapanel)
min(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltapanel)

# cat("\ndeltaimp\n")
# mean(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp))
# median(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp))
# max(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp))
# min(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp))
# 
# cat("\ndeltaimp (S.E.)\n")
# mean(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp)
# median(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp)
# max(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp)
# min(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp)

cat("\ndeltaimp 500\n")
mean(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp.500))
median(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp.500))
max(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp.500))
min(abs(p$mvLD$covstruct.mvLDSC$S_Stand.deltaimp.500))

cat("\ndeltaimp 500 (S.E.)\n")
mean(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp.500)
median(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp.500)
max(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp.500)
min(p$mvLD$covstruct.mvLDSC$S_Stand.SE.deltaimp.500)


cat("\ndeltaimp 500, covariance, rel\n")
nS<-100*(p$mvLD$covstruct.mvLDSC.1kg.500$S - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S)/abs(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S)
mean(nS)
median(nS)
max(nS)
min(nS)
...
cat("\ndeltaimp 500, covariance, rel(S.E.)\n")
nS<-100*(p$mvLD$covstruct.mvLDSC.1kg.500$S.SE - p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE)/abs(p$mvLD$covstruct.mvLDSC.1kg.ssimp$S.SE)
mean(nS)
median(nS)
max(nS)
min(nS)
...
p$sumstats.sel.table<-p$sumstats.sel[,c("name.nice.and_code",
                                        ...
                                        "h2.liability_mvLDSC.1kg.noimp",
                                        "h2.se.liability_mvLDSC.1kg.noimp",
                                        ...
                                        "h2_orig.liability_mvLDSC.1kg.noimp",
                                        "h2_orig.se.liability_mvLDSC.1kg.noimp",
...
p$sumstats.sel.table$h2.liability_mvLDSC.1kg.noimp.table<-paste0(round( p$sumstats.sel.table$h2.liability_mvLDSC.1kg.noimp,digits = 3)," (",round(p$sumstats.sel.table$h2.se.liability_mvLDSC.1kg.noimp,digits = 4),")")
...
p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.noimp.table<-paste0(round( p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.noimp,digits = 3)," (",round(p$sumstats.sel.table$h2_orig.se.liability_mvLDSC.1kg.noimp,digits = 4),")")
...
p$sumstats.sel.table<-p$sumstats.sel.table[,c("name.nice.and_code",
                                              ...
                                              "h2.liability_mvLDSC.1kg.noimp.table",
                                              "h2.liability_mvLDSC.1kg.ssimp.table",
                                              "h2.liability_mvLDSC.1kg.ldimp.table",
                                              "h2_orig.liability_mvLDSC.1kg.noimp.table",
...
p$plots.sumstats.sel.table<-p$sumstats.sel.table %>% 
  ...
  tab_header(
    ...
    h2.liability_mvLDSC.1kg.noimp.table = html("h<sup>2</sup><sub>mvldsc, 1kg</sub> (sd)"),
    ...
    h2_orig.liability_mvLDSC.1kg.noimp.table = html("h<sup>2</sup><sub>ldsc, 1kg</sub> (sd)"),
