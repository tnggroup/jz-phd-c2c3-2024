...
p$filepath.SNPReference.1kg<-normalizePath(file.path(p$folderpath.data.variantLists,"hc1kgp3.b38.mix.l2.jz2023.gz")) #Based on the high coverage 1kgp3 panel in b38
...
p$plotAndTestBatteryForMVLDSC <- function(
  ...
  title = paste0("LDSC++ intercepts, ",titleTemplate,titleAddition)
  )
...
p$printCorr(
  ...
  number.digits = 3,
  newnames = p$sumstats.sel[traitCodes,]$code.nice,
  title = paste0("LDSC++ genetic covariances, ",titleTemplate,titleAddition)
  )
...
p$printCorr(
  ...
  number.digits = 4,
  newnames = p$sumstats.sel[traitCodes,]$code.nice,
  title = paste0("LDSC++ genetic covariances S.E., ",titleTemplate,titleAddition)
  )
...
p$printCorr(
  ...
  number.digits = 3,
  newnames = p$sumstats.sel[traitCodes,]$code.nice,
  title = paste0("LDSC++ genetic correlations, ",titleTemplate,titleAddition)
  )
...
p$printCorr(
  ...
  number.digits = 4,
  newnames = p$sumstats.sel[traitCodes,]$code.nice,
  title = paste0("LDSC++ genetic correlations S.E., ",titleTemplate,titleAddition)
  )
...
if(!is.null(mvldscComparison)){
  ...
  varTest<-2*abs(mvldsc$S_Stand.SE^2-mvldscComparison$S_Stand.SE^2)
  pTest<-pnorm(q = mTest,sd = sqrt(varTest), lower.tail = T)
  ...
  #try using the #blocks as df, transform to normal - HERE!!! NOt DONE!!
  ...
  p$printCorr(
    ...
    number.digits = 3,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Difference in LDSC++ genetic correlations, ",titleTemplate,titleAdditionComparison," [abs(mean)=",round(mean(abs(dS_Stand)),digits = 2),"]")
  )
  ...
  p$printCorr(
    ...
    number.digits = 4,
    newnames = p$sumstats.sel[traitCodes,]$code.nice,
    title = paste0("Difference in LDSC++ genetic correlations S.E., ",titleTemplate,titleAdditionComparison," [mean=",round(mean(dS.S_Stand.SE),digits = 4),"]")
  )
...
p$sumstats["BODY11",]$reference.nice <- "Yengo et al." #actually, but using "Yengo et al." metadata
...
#   saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.mix ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.mix<-ldscpp(
    traits = p$sumstats.sel$mungedpath,
    sample.prev =  p$sumstats.sel$samplePrevalence,
    population.prev = p$sumstats.sel$populationPrevalence,
    trait.names = p$sumstats.sel$code,
...
p$sumstats.sel$h2.liability_mvLDSC.1kg.ldimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.05cm$S)[p$sumstats.sel$code]
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.ldimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.05cm$S.SE)[p$sumstats.sel$code]
p$sumstats.sel$h2.liability_mvLDSC.1kg.ldimp.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.05cm.small$S)[p$sumstats.sel$code]
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.ldimp.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.05cm.small$S.SE)[p$sumstats.sel$code]
...
corrForPlot <- clipValues(p$mvLD$covstruct.mvLDSC.1kg$S_Stand.unsigned,-1,1)
pForPlot <- p$mvLD$covstruct.mvLDSC.1kg$cov.p.unsigned
colnames(corrForPlot)<-p$sumstats.sel$code.nice
rownames(corrForPlot)<-p$sumstats.sel$code.nice
colnames(pForPlot)<-p$sumstats.sel$code.nice
rownames(pForPlot)<-p$sumstats.sel$code.nice
...
par(xpd=TRUE) #keep labels inside margins
pal<-colorRampPalette(c("#FF6666","#EEEEEE","#6666FF"))
corrplot(corr = corrForPlot, method = "circle", insig='blank', order = "original", p.mat = pForPlot, sig.level = c(0.05), pch.cex = 1.5, tl.cex = 0.8, full_col=FALSE, na.label = "square", na.label.col = "grey30", title = "C: LDSC++ genetic unsigned correlations, HC1kG reference:\nGWAS (non-imputed) + GWIS of male BMI (LD-IMP augmented height & weight)", tl.pos = 'd', mar=c(0,0,4,0), type = "upper", cl.pos = "r", col = pal(200))$corrPos -> p1 #type = "lower", diag=F, addCoef.col = 'black'
#text(p1$x, p1$y, round(p1$corr, 2))
p1$corr <- ifelse(0==-p1$y + ncol(corrForPlot)-p1$x + 1, NA_real_, p1$corr)
text(p1$x, p1$y, round(p1$corr, 2))
dev.off()

corrForPlot <- clipValues(p$mvLD$covstruct.mvLDSC.1kg$S_Stand.unsigned-abs(p$mvLD$covstruct.mvLDSC.1kg$S_Stand),-1,1)
pForPlot <- p$mvLD$covstruct.mvLDSC.1kg$cov.p.unsigned
colnames(corrForPlot)<-p$sumstats.sel$code.nice
rownames(corrForPlot)<-p$sumstats.sel$code.nice
colnames(pForPlot)<-p$sumstats.sel$code.nice
...
par(xpd=TRUE) #keep labels inside margins
pal<-colorRampPalette(c("#FF6666","#EEEEEE","#6666FF"))
corrplot(corr = corrForPlot, method = "circle", insig='blank', order = "original", p.mat = pForPlot, sig.level = c(0.05), pch.cex = 1.5, tl.cex = 0.8, full_col=FALSE, na.label = "square", na.label.col = "grey30", title = "D: LDSC++ 'residual' genetic correlations, HC1kG reference:\nLD-IMP augmented GWAS + GWIS of male BMI (LD-IMP augmented height & weight)", tl.pos = 'd', mar=c(0,0,4,0), type = "upper", cl.pos = "r", col = pal(200))$corrPos -> p1 #type = "lower", diag=F, addCoef.col = 'black'
#text(p1$x, p1$y, round(p1$corr, 2))
p1$corr <- ifelse(0==-p1$y + ncol(corrForPlot)-p1$x + 1, NA_real_, p1$corr)
text(p1$x, p1$y, round(p1$corr, 2))
dev.off()
...
p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.original, code = "original",titleAddition = "original LDSC (Genomic SEM)")

p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs, code = "vbcs",titleAddition = "VBCS",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.original,titleAdditionComparison = "comparing with original LDSC (Genomic SEM) settings")

infoTraits<-p$sumstats[which(p$sumstats$hasinfo==T),]$code
p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.nocm, code = "vbcs.nocm",titleAddition = "No cM based VBCS",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs, testOnlyTraitNameCodes = infoTraits, titleAdditionComparison = "No cM based VBCS comparing with cM based VBCS only")

infoTraits<-p$sumstats[which(p$sumstats$hasinfo==T),]$code
p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.winfo, code = "vbcs.winfo",titleAddition = "VBCS + WINFO",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs, testOnlyTraitNameCodes = infoTraits, titleAdditionComparison = "VBCS + WINFO comparing with  VBCS only")
...
p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg, code = "panel",titleAddition = "HC1kG",mvldscComparison = p$mvLD$covstruct.mvLDSC.hm3,titleAdditionComparison = "comparing with HM3")

#GWAS summary statistics imputation comparison
imptraits<-p$sumstats.sel[which(p$sumstats.sel$code!="BODYT2MI"),]$code
p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.05cm, code = "ldimp",titleAddition = "0.5cM imputation window",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg, testOnlyTraitNameCodes = imptraits,titleTemplate = "HC1kG reference,\nLD-IMP augmented GWAS + GWIS of male BMI (LD-IMP augmented height & weight),\n", titleAdditionComparison = "comparing with LDSC++ VBCS + DRC + WINFO of non-imputed GWAS")

p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.500, code = "ldimp.500kb",titleAddition = "500kb imputation window",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg, testOnlyTraitNameCodes = imptraits,titleTemplate = "HC1kG reference,\nLD-IMP augmented GWAS + GWIS of male BMI (LD-IMP augmented height & weight),\n",titleAdditionComparison = "comparing with LDSC++ of non-imputed GWAS")

imptraits<-p$sumstats.sel[which(p$sumstats.sel$code!="BODYT2MI"),]$code
p$plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.05cm, code = "ldimp.frame",titleAddition = "0.5cM imputation window",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.500, testOnlyTraitNameCodes = imptraits,titleTemplate = "HC1kG reference,\nLD-IMP augmented GWAS + GWIS of male BMI (LD-IMP augmented height & weight),\n", titleAdditionComparison = "comparing 0.5cM with 500kb imputation window")
