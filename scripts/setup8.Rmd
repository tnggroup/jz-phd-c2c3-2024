...
#devtools::install_github("tnggroup/genetic_correlations")
...
p$filepath.SNPReference.gladp<-normalizePath(file.path(p$folderpath.data.variantLists,"GLAD_EDGI_NBR.mix.l2.jz2024.gz")) 
...
p$folderpath.data.mvLDSC.ld.GLADp <- file.path(p$folderpath.data,"ld_scores","w_ld.GLAD_EDGI_NBR.keep.b38.gcta")
...
p$folderpath.data.mvLDSC.wld.GLADp <- p$folderpath.data.mvLDSC.ld.GLADp
...
p$sumstats.sel.sim.code<-c("SMRV01","SMRV02","SMRV03","SMRV04","SMRV05","SMRV06","SMRV07","SMRV08")
...
if(p$clOptions$task=="sim") {
  ...
  #we do not have the in-sample LD in the 1kG variant list so we need to complete with these LD scores.
  inSampleLDScores<-shru::readFile(filePath = file.path(p$folderpath.data.mvLDSC.ld.GLADp,"GLAD_EDGI_NBR.keep.b38.gcta.l2.ldscore.gz"), nThreads = p$nThread) #hard coded
  
  #L2 info
  #update LD scores
  variantsToSimulate[,L2:=NA]
  setkeyv(variantsToSimulate, cols = c("SNP"))
  setkeyv(inSampleLDScores, cols = c("SNP"))
  variantsToSimulate[inSampleLDScores,on=c('SNP'), c("L2"):=list(i.L2)]
  nrow(variantsToSimulate[!is.na(L2),])
  rm(inSampleLDScores)
  
  L2.mean <- mean(variantsToSimulate$L2,na.rm=T)
  L2.median <- median(variantsToSimulate$L2,na.rm=T)
  
  k.number_of_polygenic_variants<-30000
  
  fractionCommonPolygenic<-k.number_of_polygenic_variants/nrow(variantsToSimulate[MAF >0.01,])
  
  #select a random set of variants to achieve targeted size
  variantsToSimulate[MAF >0.01,c("SELQ1")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic) #1, 0.75, 0.5, 0.25, 0.6, (0.8)
  variantsToSimulate[MAF >0.01,c("SELQ2")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  variantsToSimulate[MAF >0.01,c("SELQ3")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  variantsToSimulate[MAF >0.01,c("SELQ4")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  
  variantsToSimulate[MAF >0.01,c("SELQ1.1")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic) #1, 0.75, 0.5, 0.25, 0.6, (0.8)
  variantsToSimulate[MAF >0.01,c("SELQ2.1")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  variantsToSimulate[MAF >0.01,c("SELQ3.1")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  variantsToSimulate[MAF >0.01,c("SELQ4.1")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  
  variantsToSimulate[MAF >0.01,c("SELQ1.2")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic) #1, 0.75, 0.5, 0.25, 0.6, (0.8)
  variantsToSimulate[MAF >0.01,c("SELQ2.2")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  variantsToSimulate[MAF >0.01,c("SELQ3.2")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  variantsToSimulate[MAF >0.01,c("SELQ4.2")]<-rbinom(nrow(variantsToSimulate[MAF >0.01,]), 1, 0.25*fractionCommonPolygenic)
  
  nrow(variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4 > 0 & MAF >0.01,])
  ...
  ##100% overlap
  simulatedVars.true.rg <- cor(variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4 > 0 & MAF >0.01,c("RV1","RV2")])
  print(simulatedVars.true.rg)
#          RV1       RV2
# RV1 1.0000000 0.3944149
# RV2 0.3944149 1.0000000
  simulatedVars.true.covg <- cov(variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4 > 0 & MAF >0.01,c("RV1","RV2")])
  print(simulatedVars.true.covg)
#            RV1        RV2
# RV1 0.19995512 0.07831909
# RV2 0.07831909 0.19719507
# 
#   ##75% overlap
#   simulatedVars.true.rg <- cor(variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4.1 + SELQ4.2 > 0 & MAF >0.01,c("RV1","RV2")])
#   print(simulatedVars.true.rg)
#   simulatedVars.true.covg <- cov(variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4.1 + SELQ4.2 > 0 & MAF >0.01,c("RV1","RV2")])
#   print(simulatedVars.true.covg)
# 
#   ##50% overlap
#   simulatedVars.true.rg <- cor(variantsToSimulate[SELQ1 + SELQ2 + SELQ3.1 + SELQ4.1 + SELQ3.2 + SELQ4.2 > 0 & MAF >0.01,c("RV1","RV2")])
#   print(simulatedVars.true.rg)
#   simulatedVars.true.covg <- cov(variantsToSimulate[SELQ1 + SELQ2 + SELQ3.1 + SELQ4.1 + SELQ3.2 + SELQ4.2 > 0 & MAF >0.01,c("RV1","RV2")])
#   print(simulatedVars.true.covg)
# 
#   ##25% overlap
#   simulatedVars.true.rg <- cor(variantsToSimulate[SELQ1 + SELQ2.1 + SELQ3.1 + SELQ4.1 + SELQ2.2 + SELQ3.2 + SELQ4.2 > 0 & MAF >0.01,c("RV1","RV2")])
#   print(simulatedVars.true.rg)
#   simulatedVars.true.covg <- cov(variantsToSimulate[SELQ1 + SELQ2.1 + SELQ3.1 + SELQ4.1 + SELQ2.2 + SELQ3.2 + SELQ4.2 > 0 & MAF >0.01,c("RV1","RV2")])
#   print(simulatedVars.true.covg)
...
#   #L2 - uncomment this if not simulating explicit LD effects
#   ##QC
#   #nrow(inSampleLDScores[MAF>0.05,])
#   #LDSC.M<-7184778 #the LDSC 'M' value (#variants in LD score panel with MAF > 0.05)
#   #LDSC.M<-6299745 #the LDSC 'M' value (#variants in LD score panel with MAF > 0.05) #These are redundant when performing the scaling below
#   #[,L:=sqrt(L2)]
...
#   variantsToSimulate[,EFFECT1.LD:=EFFECT1*L][,EFFECT2.LD:=EFFECT2*L] #linear version - CURRENT version (ld)
#   #scale back to approximate previous heritability
...
# #            EFFECT1.LD EFFECT2.LD
# # EFFECT1.LD 0.23963802 0.09572032
# # EFFECT2.LD 0.09572032 0.23966618
  
 fwrite(x = variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4 > 0 & MAF >0.01,.(SNP,EFFECT1)], file = file.path(p$folderpath.workingDirectory,"simEffects1.020.100"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  fwrite(x = variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4 > 0 & MAF >0.01,.(SNP,EFFECT2)], file = file.path(p$folderpath.workingDirectory,"simEffects2.020.100"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  
 fwrite(x = variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4.1 > 0 & MAF >0.01,.(SNP,EFFECT1)], file = file.path(p$folderpath.workingDirectory,"simEffects1.020.75"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  fwrite(x = variantsToSimulate[SELQ1 + SELQ2 + SELQ3 + SELQ4.2 > 0 & MAF >0.01,.(SNP,EFFECT2)], file = file.path(p$folderpath.workingDirectory,"simEffects2.020.75"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  
 fwrite(x = variantsToSimulate[SELQ1 + SELQ2 + SELQ3.1 + SELQ4.1 > 0 & MAF >0.01,.(SNP,EFFECT1)], file = file.path(p$folderpath.workingDirectory,"simEffects1.020.50"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  fwrite(x = variantsToSimulate[SELQ1 + SELQ2 + SELQ3.2 + SELQ4.2 > 0 & MAF >0.01,.(SNP,EFFECT2)], file = file.path(p$folderpath.workingDirectory,"simEffects2.020.50"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  
 fwrite(x = variantsToSimulate[SELQ1 + SELQ2.1 + SELQ3.1 + SELQ4.1 > 0 & MAF >0.01,.(SNP,EFFECT1)], file = file.path(p$folderpath.workingDirectory,"simEffects1.020.25"), col.names = FALSE,nThread = p$nThread, sep = "\t")
  fwrite(x = variantsToSimulate[SELQ1 + SELQ2.2 + SELQ3.2 + SELQ4.2 > 0 & MAF >0.01,.(SNP,EFFECT2)], file = file.path(p$folderpath.workingDirectory,"simEffects2.020.25"), col.names = FALSE,nThread = p$nThread, sep = "\t")
...
pheno1 <- shru::readFile(filePath = file.path(p$folderpath.workingDirectory,"simTrait1.020.25.phen"),nThreads = p$nThread)

pheno2 <- shru::readFile(filePath = file.path(p$folderpath.workingDirectory,"simTrait2.020.25.phen"),nThreads = p$nThread)

pheno1[pheno2,on=c("V1"),V4:=i.V3]

fwrite(x = pheno1,file = file.path(p$folderpath.workingDirectory,"simTrait1and2.020.25.phen"),col.names = F, sep = "\t")
...
if(p$clOptions$task=="mvLD" || (!file.exists(p$filepath.mvLD) & p$clOptions$task=="0"))
{
  ...
  #   filter.maf = 0.01,
  #   stand = T,
  #   force.M = 6369726,  #from previous ldscpp run logs
  #   do.filter.unique.SNP = F, #we do not want to filter these as we did not for the real world data
  #   ldsc.log = p$setup.code.date
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.GSEMmvLDSC.gladp.sim ***\n")
...
# 
  #this is just called 1kg because of legacy
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.varblock.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.varblock.sim ***\n")
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.rblock.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
    ld = p$folderpath.data.mvLDSC.ld.GLADp,
    filepathVariantsAdditional = p$filepath.SNPReference.gladp,
    ldsc.log = p$setup.code.date,
    preweight.alternativeCorrelationCorrection = F,
    preweight.ChiSquare = F,
    correctAttenuationBias = F,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.rblock.sim ***\n")
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.varblock.altcw.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.varblock.altcw.sim ***\n")
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.rblock.altcw.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.rblock.altcw.sim ***\n")
  ...
  #DO NOT USE
  # cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.rblock.unfiltered.sim ***\n")
  # p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.unfiltered.sim<-ldscpp(
  #   traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
  #   sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.sim$populationPrevalence,
  #   trait.names = p$sumstats.sel.sim$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.rblock.unfiltered.sim ***\n")
  ...
  #DO NOT USE
  # cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.varblock.winfo.sim ***\n")
  # p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.winfo.sim<-ldscpp(
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.varblock.winfo.sim ***\n")
  ...
  #DO NOT USE
  # cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.jn.rblock.winfo.sim ***\n")
  # p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.winfo.sim<-ldscpp(
  #   traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered, #we need all columns - INFO
  #   sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.sim$populationPrevalence,
  #   trait.names = p$sumstats.sel.sim$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.jn.rblock.winfo.sim ***\n")
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.sim ***\n")
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.sim ***\n")
  ...
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim ***\n")
  ...
  # 
  # 
  cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim ***\n")
  p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim<-ldscpp(
    traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
    sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
    population.prev = p$sumstats.sel.sim$populationPrevalence,
    trait.names = p$sumstats.sel.sim$code,
  ...
  saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim ***\n")
  # 
  # 
  # 
  # 
  ...
  #DO NOT USE
  # cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.unfiltered.sim ***\n")
  # p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.unfiltered.sim<-ldscpp(
  #   traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered,
  #   sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.sim$populationPrevalence,
  #   trait.names = p$sumstats.sel.sim$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.unfiltered.sim ***\n")
  ...
  #DO NOT USE
  # cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.sim ***\n")
  # p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.winfo.sim<-ldscpp(
  #   traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered, #we need all columns - INFO
  #   sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.sim$populationPrevalence,
  #   trait.names = p$sumstats.sel.sim$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.varblock.winfo.sim ***\n")
  ...
  #DO NOT USE
  # cat("\n\n*** LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.winfo.sim ***\n")
  # p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.winfo.sim<-ldscpp(
  #   traits = p$sumstats.sel.sim$mungedpath.supermunge.1kg.orig.unfiltered, #we need all columns - INFO
  #   sample.prev =  p$sumstats.sel.sim$samplePrevalence.balanced,
  #   population.prev = p$sumstats.sel.sim$populationPrevalence,
  #   trait.names = p$sumstats.sel.sim$code,
  ...
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  # cat("\n\n***END LDSC: covstruct.mvLDSC.1kg.vbcs.rblock.winfo.sim ***\n")
  ...
  #   filter.maf = 0.01,
  #   filter.zz.min = 0
  #   #force.M = 7184778 #do not use this when using GLAD ld scores as we do not have the MAF>0.05 M
  #   )
  # saveRDS(object = p$mvLD,file = p$filepath.mvLD)
  ...
  #USE THE altcw VERSION INSTEAD
  # #test with double regression - method needs a go-through in the function!!!
...
#test of heteroskedasticity in LDSR results, using Genomic SEM LDSC settings

p$dfHetTest<-data.frame(S=p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S[lower.tri(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S,diag = T)], S.SE=p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE[lower.tri(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE,diag = T)])

##only heritabilities
cor.test(diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S),diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE))

#plot(diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S),diag(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE))

##all covariances
cor.test(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S[lower.tri(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S,diag = T)],p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE[lower.tri(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE,diag = T)])

#plot(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S[lower.tri(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S,diag = T)],p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE[lower.tri(p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01$S.SE,diag = T)])
  
```

#Hard coded GCTA covG
These are obtained from GCTA hsq-files.
```{r hard coded GCTA results}

p$mvLD$covstruct.GCTA<-c()
p$mvLD$covstruct.GCTA$S<-matrix(data=NA,nrow=8,ncol = 8)
colnames(p$mvLD$covstruct.GCTA$S)<-colnames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
rownames(p$mvLD$covstruct.GCTA$S)<-rownames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
p$mvLD$covstruct.GCTA$S.SE<-matrix(data=NA,nrow=8,ncol = 8)
colnames(p$mvLD$covstruct.GCTA$S.SE)<-colnames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
rownames(p$mvLD$covstruct.GCTA$S.SE)<-rownames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
p$mvLD$covstruct.GCTA$S_Stand<-matrix(data=NA,nrow=8,ncol = 8)
colnames(p$mvLD$covstruct.GCTA$S_Stand)<-colnames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
rownames(p$mvLD$covstruct.GCTA$S_Stand)<-rownames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
p$mvLD$covstruct.GCTA$S_Stand.SE<-matrix(data=NA,nrow=8,ncol = 8)
colnames(p$mvLD$covstruct.GCTA$S_Stand.SE)<-colnames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)
rownames(p$mvLD$covstruct.GCTA$S_Stand.SE)<-rownames(p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim$S)

#values
p$mvLD$covstruct.GCTA$S["SMRV01","SMRV01"]<-0.127665
p$mvLD$covstruct.GCTA$S.SE["SMRV01","SMRV01"]<-0.014984
p$mvLD$covstruct.GCTA$S["SMRV02","SMRV02"]<-0.151516
p$mvLD$covstruct.GCTA$S.SE["SMRV02","SMRV02"]<-0.015225
p$mvLD$covstruct.GCTA$S_Stand["SMRV01","SMRV02"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV02","SMRV01"]<-0.435028
p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV01","SMRV02"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV02","SMRV01"]<-0.079526
p$mvLD$covstruct.GCTA$S["SMRV01","SMRV02"]<-p$mvLD$covstruct.GCTA$S["SMRV02","SMRV01"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV01","SMRV02"]*sqrt(p$mvLD$covstruct.GCTA$S["SMRV01","SMRV01"])*sqrt(p$mvLD$covstruct.GCTA$S["SMRV02","SMRV02"])
p$mvLD$covstruct.GCTA$S.SE["SMRV01","SMRV02"]<-p$mvLD$covstruct.GCTA$S.SE["SMRV02","SMRV01"]<-(p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV01","SMRV02"]/p$mvLD$covstruct.GCTA$S_Stand["SMRV01","SMRV02"])*p$mvLD$covstruct.GCTA$S["SMRV01","SMRV02"]

p$mvLD$covstruct.GCTA$S["SMRV03","SMRV03"]<-0.151941
p$mvLD$covstruct.GCTA$S.SE["SMRV03","SMRV03"]<-0.015072
p$mvLD$covstruct.GCTA$S["SMRV04","SMRV04"]<-0.141567
p$mvLD$covstruct.GCTA$S.SE["SMRV04","SMRV04"]<-0.015106
p$mvLD$covstruct.GCTA$S_Stand["SMRV03","SMRV04"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV04","SMRV03"]<-0.240095
p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV03","SMRV04"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV04","SMRV03"]<-0.073349
p$mvLD$covstruct.GCTA$S["SMRV03","SMRV04"]<-p$mvLD$covstruct.GCTA$S["SMRV04","SMRV03"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV03","SMRV04"]*sqrt(p$mvLD$covstruct.GCTA$S["SMRV03","SMRV03"])*sqrt(p$mvLD$covstruct.GCTA$S["SMRV04","SMRV04"])
p$mvLD$covstruct.GCTA$S.SE["SMRV03","SMRV04"]<-p$mvLD$covstruct.GCTA$S.SE["SMRV04","SMRV03"]<-(p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV03","SMRV04"]/p$mvLD$covstruct.GCTA$S_Stand["SMRV03","SMRV04"])*p$mvLD$covstruct.GCTA$S["SMRV03","SMRV04"]

p$mvLD$covstruct.GCTA$S["SMRV05","SMRV05"]<-0.148363
p$mvLD$covstruct.GCTA$S.SE["SMRV05","SMRV05"]<-0.015026
p$mvLD$covstruct.GCTA$S["SMRV06","SMRV06"]<-0.152252
p$mvLD$covstruct.GCTA$S.SE["SMRV06","SMRV06"]<-0.015090
p$mvLD$covstruct.GCTA$S_Stand["SMRV05","SMRV06"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV06","SMRV05"]<-0.193809
p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV05","SMRV06"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV06","SMRV05"]<-0.071777
p$mvLD$covstruct.GCTA$S["SMRV05","SMRV06"]<-p$mvLD$covstruct.GCTA$S["SMRV06","SMRV05"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV05","SMRV06"]*sqrt(p$mvLD$covstruct.GCTA$S["SMRV05","SMRV05"])*sqrt(p$mvLD$covstruct.GCTA$S["SMRV06","SMRV06"])
p$mvLD$covstruct.GCTA$S.SE["SMRV05","SMRV06"]<-p$mvLD$covstruct.GCTA$S.SE["SMRV06","SMRV05"]<-(p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV05","SMRV06"]/p$mvLD$covstruct.GCTA$S_Stand["SMRV05","SMRV06"])*p$mvLD$covstruct.GCTA$S["SMRV05","SMRV06"]

p$mvLD$covstruct.GCTA$S["SMRV07","SMRV07"]<-0.146128
p$mvLD$covstruct.GCTA$S.SE["SMRV07","SMRV07"]<-0.015038
p$mvLD$covstruct.GCTA$S["SMRV08","SMRV08"]<-0.143366
p$mvLD$covstruct.GCTA$S.SE["SMRV08","SMRV08"]<-0.014972
p$mvLD$covstruct.GCTA$S_Stand["SMRV07","SMRV08"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV08","SMRV07"]<-0.202540
p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV07","SMRV08"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV08","SMRV07"]<-0.074658
p$mvLD$covstruct.GCTA$S["SMRV07","SMRV08"]<-p$mvLD$covstruct.GCTA$S["SMRV08","SMRV07"]<-p$mvLD$covstruct.GCTA$S_Stand["SMRV07","SMRV08"]*sqrt(p$mvLD$covstruct.GCTA$S["SMRV07","SMRV07"])*sqrt(p$mvLD$covstruct.GCTA$S["SMRV08","SMRV08"])
p$mvLD$covstruct.GCTA$S.SE["SMRV07","SMRV08"]<-p$mvLD$covstruct.GCTA$S.SE["SMRV08","SMRV07"]<-(p$mvLD$covstruct.GCTA$S_Stand.SE["SMRV07","SMRV08"]/p$mvLD$covstruct.GCTA$S_Stand["SMRV07","SMRV08"])*p$mvLD$covstruct.GCTA$S["SMRV07","SMRV08"]
...
# Overview of LD score regression CV's , Simulated traits, GLAD+, NEW - 
```{r overview ldsc cv SIM NEW GLADp, fig.width=9, fig.height=6, out.width="1600px", out.height="1000px", eval=FALSE, purl=FALSE}


#individual methods
p$cv_coverage.ldsc.1kg.gsem <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim, labelString = "1kg.gsem", groupString = "1kg.gsem")

p$cv_coverage.ldsc.1kg.jn.varblock <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim, labelString = "1kg.jn.varblock", groupString = "1kg.jn.varblock")

p$cv_coverage.ldsc.1kg.jn.rblock <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim, labelString = "1kg.jn.rblock", groupString = "1kg.jn.rblock")

p$cv_coverage.ldsc.1kg.vbcs.varblock <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim, labelString = "1kg.vbcs.varblock", groupString = "1kg.vbcs.varblock")

p$cv_coverage.ldsc.1kg.vbcs.rblock <- p$getCVDataframe(cCovstruct = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim, labelString = "1kg.vbcs.rblock", groupString = "1kg.vbcs.rblock")
...
ggplot(data = p$cv_coverage, aes(x=m, y=cv, shape=l, color=g)) +
  ...
  stat_cor(method="pearson", label.x.npc = 0.3,label.y.npc = 0.9, aes(group=g))
  #stat_cor(method="pearson", label.x.npc = "middle",label.y.npc = "bottom")
ggsave(filename=file.path(p$folderpath.plots,"ldsc_m_cv.sim.covG.png"), plot = last_plot(), width = 200, height = 200, units = "mm")
...
ggplot(data = p$cv_coverage[p$cv_coverage$isDiag==T,], aes(x=m, y=cv, shape=l, color=g)) +
  ...
  stat_cor(method="pearson", label.x.npc = 0.3,label.y.npc = 1, aes(group=g))
  #stat_cor(method="pearson", label.x.npc = "middle",label.y.npc = "bottom")
ggsave(filename=file.path(p$folderpath.plots,"ldsc_m_cv.sim.h2.png"), plot = last_plot(), width = 200, height = 200, units = "mm")
...
#we only have diagonals for original LDSC
...
batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim, folderpath.plots = p$folderpath.plots, code = "original.gladp.sim.gsem", titleTemplate = "LDSC++ (Genomic SEM emulated), GLAD+ reference", titleAddition = "\nBlock Jackknife Resampling", mvldscComparison =semplate$amendGsemLDSC(  p$mvLD$covstruct.GSEMmvLDSC.gladp.sim),titleAdditionComparison = " vs original Genomic SEM LDSC", df.summary=batRes$df.summary)
...
#simulated
batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs.v.now", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs LDSC++ (Genomic SEM emulated) 200 blocks", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs.r.now", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs LDSC++ (Genomic SEM emulated) 200 blocks", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.jn.v.now", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nblock jackknife resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs LDSC++ (Genomic SEM emulated) 200 blocks", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.jn.r.now", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nblock jackknife resampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs LDSC++ (Genomic SEM emulated) 200 blocks", df.summary=batRes$df.summary)
...
#simulated data
batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs.v", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs original LDSC (Genomic SEM emulated)", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs.r", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs original LDSC (Genomic SEM emulated)", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.jn.v", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nBlock Jackknife Resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs original LDSC (Genomic SEM emulated)", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.jn.r", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nBlock Jackknife Resampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,titleAdditionComparison = " vs original LDSC (Genomic SEM emulated)", df.summary=batRes$df.summary)

plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs_jn", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,titleAdditionComparison = " vs Block Jackknife Resampling", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs.r_jn", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, recombination distance blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,titleAdditionComparison = " vs Block Jackknife Resampling", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.vbcs.v_r", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nVariable Block-Count Sampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,titleAdditionComparison = " vs recombination distance blocks", df.summary=batRes$df.summary)

batRes <- plotAndTestBatteryForMVLDSC(mvldsc = p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim, folderpath.plots = p$folderpath.plots, code = "gladp.sim.jn.v_r", titleTemplate = "LDSC++, GLAD+ reference", titleAddition = "\nblock jackknife resampling, #variants blocks",mvldscComparison = p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,titleAdditionComparison = " vs recombination distance blocks", df.summary=batRes$df.summary)
...
#gt-table: real world data results table for manuscript

p$df.summary.rw_data.table<-batRes$df.summary[
  c(
    "1kg.vbcs.v",
    "1kg.vbcs.r",
    "1kg.jn.v",
...
p$df.summary.rw_data.table$code<-rownames(p$df.summary.rw_data.table)
p$df.summary.rw_data.table["1kg.vbcs.v","code"]<-"vbcs.v"
p$df.summary.rw_data.table["1kg.vbcs.r","code"]<-"vbcs.r"
p$df.summary.rw_data.table["1kg.jn.v","code"]<-"jn.v"
p$df.summary.rw_data.table["1kg.jn.r","code"]<-"jn.r"

p$df.summary.rw_data.table %>% 
  dplyr::select(
    code,
    #mDeltaCovG,
    #mDeltaCovG.h2,
...
# Create comparison table for simulated trait results, covG
```{r comparison table for simulated trait results, fig.width=9, fig.height=6, out.width="1600px", out.height="1000px", eval=FALSE, purl=FALSE}

simulatedTraitsTableS<-as.data.frame(matrix(data=NA,nrow = 0,ncol = 0))
ltCovstructS<-as.matrix(Matrix::tril(p$mvLD$covstruct.GCTA$S)) #something goes wrong with using lower.tri here
ltCovstructS[upper.tri(ltCovstructS)]<-NA
#View(ltCovstructS)
diagCovstructS<-p$mvLD$covstruct.GCTA$S
diagCovstructS[upper.tri(diagCovstructS)]<-NA
diagCovstructS[lower.tri(diagCovstructS)]<-NA
offDiagCovstructS<-ltCovstructS
diag(offDiagCovstructS)<-NA


#test
#code<-"gladp.sim.gsem"
#cCovstruct <-p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim
#simulatedTraitsTable<-simulatedTraitsTableS
createTableEntriesFull<-function(code,cCovstruct,simulatedTraitsTable){
  cols <-c("Avg. SIM1 h2",
           "Avg. SIM2 h2",
           "Avg. covG",
           "Avg. diag. covG",
           "Avg. off diag. covG",
           "Avg. covG difference (%) rel. REML",
           "Avg. diag. covG difference (%) rel. REML",
           "Avg. off diag. covG difference (%) rel. REML",
           "RMSE(covG)",
           "RMSE(diag. covG)",
           "RMSE(off diag. covG)")
  simulatedTraitsTable[code,cols]<-c(
  
  (cCovstruct$S["SMRV01","SMRV01"] + cCovstruct$S["SMRV03","SMRV03"] +
    cCovstruct$S["SMRV05","SMRV05"] + cCovstruct$S["SMRV07","SMRV07"])/4,

  (cCovstruct$S["SMRV02","SMRV02"] + cCovstruct$S["SMRV04","SMRV04"] +
    cCovstruct$S["SMRV06","SMRV06"] + cCovstruct$S["SMRV08","SMRV08"])/4,
  
  mean(cCovstruct$S,na.rm=T),

  mean(diag(cCovstruct$S),na.rm=T),
  
  (cCovstruct$S["SMRV01","SMRV02"] + cCovstruct$S["SMRV03","SMRV04"] + 
    cCovstruct$S["SMRV05","SMRV06"] + cCovstruct$S["SMRV07","SMRV08"])/4,
  
  mean(100*(cCovstruct$S-ltCovstructS)/ltCovstructS,na.rm=T),
  mean(100*(cCovstruct$S-diagCovstructS)/diagCovstructS,na.rm=T),
  mean(100*(cCovstruct$S-offDiagCovstructS)/offDiagCovstructS,na.rm=T),
  
  sqrt(mean((cCovstruct$S-ltCovstructS)^2,na.rm=T)),
  sqrt(mean((cCovstruct$S-diagCovstructS)^2,na.rm=T)),
  sqrt(mean((cCovstruct$S-offDiagCovstructS)^2,na.rm=T))
  
)
  simulatedTraitsTable
}

#**FULL RESULT**

#GCTA REML
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS)

#emulated standard LDSC
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS)

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS)

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS)

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS)

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS)

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS)

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS)

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS)

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesFull("gladp.sim.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS)



createTableEntriesPairwise<-function(code,cCovstruct,simulatedTraitsTable,t1,t2){
  cols <-c("Avg. SIM1 h2",
           "Avg. SIM2 h2",
           "Avg. covG",
           "Avg. diag. covG",
  ...
  mean(cCovstruct$S[c(t1,t2),c(t1,t2)],na.rm=T),
  
  mean(diag(cCovstruct$S[c(t1,t2),c(t1,t2)]),na.rm=T),
  
  cCovstruct$S[t1,t2],
  
  mean(100*(cCovstruct$S[c(t1,t2),c(t1,t2)]-ltCovstructS[c(t1,t2),c(t1,t2)])/ltCovstructS[c(t1,t2),c(t1,t2)],na.rm=T),
  mean(100*(cCovstruct$S[c(t1,t2),c(t1,t2)]-diagCovstructS[c(t1,t2),c(t1,t2)])/diagCovstructS[c(t1,t2),c(t1,t2)],na.rm=T),
  mean(100*(cCovstruct$S[c(t1,t2),c(t1,t2)]-offDiagCovstructS[c(t1,t2),c(t1,t2)])/offDiagCovstructS[c(t1,t2),c(t1,t2)],na.rm=T),
  
  sqrt(mean((cCovstruct$S[c(t1,t2),c(t1,t2)]-ltCovstructS[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  sqrt(mean((cCovstruct$S[c(t1,t2),c(t1,t2)]-diagCovstructS[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  sqrt(mean((cCovstruct$S[c(t1,t2),c(t1,t2)]-offDiagCovstructS[c(t1,t2),c(t1,t2)])^2,na.rm=T))
...
#GCTA REML
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS,"SMRV01","SMRV02")

#emulated standard LDSC
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim100.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS,"SMRV01","SMRV02")

#**75% OVERLAP**

#GCTA REML
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS,"SMRV03","SMRV04")

#emulated standard LDSC
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim75.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS,"SMRV03","SMRV04")

#**50% OVERLAP**

#GCTA REML
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS,"SMRV05","SMRV06")

#emulated standard LDSC
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim50.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS,"SMRV05","SMRV06")


#**25% OVERLAP**

#GCTA REML
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS,"SMRV07","SMRV08")

#emulated standard LDSC
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS<-createTableEntriesPairwise("gladp.sim25.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS,"SMRV07","SMRV08")

#to spreadsheet by View(simulatedTraitsTableS)



#run pretend GCTA covariance structure in test pipeline
pretendGCTAcovstruct <- p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim
pretendGCTAcovstruct$S<-p$mvLD$covstruct.GCTA$S
pretendGCTAcovstruct$S.SE<-p$mvLD$covstruct.GCTA$S.SE
pretendGCTAcovstruct$S_Stand<-p$mvLD$covstruct.GCTA$S_Stand
pretendGCTAcovstruct$S_Stand.SE<-p$mvLD$covstruct.GCTA$S_Stand.SE
pretendGCTAcovstruct$S[is.na(pretendGCTAcovstruct$S)]<-0
pretendGCTAcovstruct$S.SE[is.na(pretendGCTAcovstruct$S.SE)]<-0
pretendGCTAcovstruct$S_Stand[is.na(pretendGCTAcovstruct$S_Stand)]<-0
pretendGCTAcovstruct$S_Stand.SE[is.na(pretendGCTAcovstruct$S_Stand.SE)]<-0

plotAndTestBatteryForMVLDSC(mvldsc = pretendGCTAcovstruct, folderpath.plots = p$folderpath.plots, code = "gladp.gcta.reml", titleTemplate = "GCTA REML, GLAD+ reference", titleAddition = "")
...
# Create comparison table for simulated trait results, covG SE
```{r comparison table for simulated trait results covg se, fig.width=9, fig.height=6, out.width="1600px", out.height="1000px", eval=FALSE, purl=FALSE}

simulatedTraitsTableS.SE<-as.data.frame(matrix(data=NA,nrow = 0,ncol = 0))

ltCovstructS.SE<-as.matrix(Matrix::tril(p$mvLD$covstruct.GCTA$S.SE)) #something goes wrong with using lower.tri here
ltCovstructS.SE[upper.tri(ltCovstructS.SE)]<-NA

diagCovstructS.SE<-p$mvLD$covstruct.GCTA$S.SE
diagCovstructS.SE[upper.tri(diagCovstructS.SE)]<-NA
diagCovstructS.SE[lower.tri(diagCovstructS.SE)]<-NA
offDiagCovstructS.SE<-ltCovstructS.SE
diag(offDiagCovstructS.SE)<-NA

#needs the computations for the comparison table above
ltCovstructS_ForCV<-abs(ltCovstructS)
ltCovstruct_g_se <- sqrt(diag(ltCovstructS_ForCV))
ltCovstructS_se2 <- ltCovstruct_g_se %*% t(ltCovstruct_g_se) #this is the diagonal based genetic covariance
condCap<-(ltCovstructS_ForCV < ltCovstructS_se2 * 0.05)
condCap[is.na(condCap)]<-F
ltCovstructS_ForCV[condCap] <- (ltCovstructS_se2 * 0.05)[condCap] #cap at min rg 0.05 to avoid extreme values
ltCovstructS.CV<-(ltCovstructS.SE/ltCovstructS_ForCV)
diagCovstructS.CV<-ltCovstructS.CV
diagCovstructS.CV[upper.tri(diagCovstructS.CV)]<-NA
diagCovstructS.CV[lower.tri(diagCovstructS.CV)]<-NA
offDiagCovstructS.CV<-ltCovstructS.CV
diag(offDiagCovstructS.CV)<-NA

createTableEntriesFull<-function(code,cCovstruct,simulatedTraitsTable){
   cols <-c("Avg. SIM1 h2 SE",
            "Avg. SIM2 h2 SE",
            "Avg. covG SE",
            "Avg. diag. covG SE",
            "Avg. off diag. SE",
            "Avg. covG SE difference (%) rel. REML",
            "Avg. diag. covG SE difference (%) rel. REML",
            "Avg. off diag. covG SE difference (%) rel. REML",
            "RMSE(covG SE)",
            "RMSE(diag. covG SE)",
            "RMSE(off diag. covG SE)",
            "Avg. SIM1 h2 CV",
            "Avg. SIM2 h2 CV",
            "Avg. covG CV",
            "Avg. diag. covG CV",
            "Avg. off diag. CV",
            "Avg. covG CV difference (%) rel. REML",
            "Avg. diag. covG CV difference (%) rel. REML",
            "Avg. off diag. covG CV difference (%) rel. REML",
            "RMSE(covG CV)",
            "RMSE(diag. covG CV)",
            "RMSE(off diag. covG CV)")

  S_ForCV<-abs(cCovstruct$S)
  g_se <- sqrt(diag(S_ForCV))
  S_se2 <- g_se %*% t(g_se) #this is the diagonal based genetic covariance
  condCap<-(S_ForCV < S_se2 * 0.05)
  condCap[is.na(condCap)]<-F
  S_ForCV[condCap] <- (S_se2 * 0.05)[condCap] #cap at min rg 0.05 to avoid extreme values
  S.CV<-(cCovstruct$S.SE/S_ForCV)
   
  simulatedTraitsTable[code,cols]<-c(
  
  (cCovstruct$S.SE["SMRV01","SMRV01"] + cCovstruct$S.SE["SMRV03","SMRV03"] + 
    cCovstruct$S.SE["SMRV05","SMRV05"] + cCovstruct$S.SE["SMRV07","SMRV07"])/4,
  
  (cCovstruct$S.SE["SMRV02","SMRV02"] + cCovstruct$S.SE["SMRV04","SMRV04"] + 
    cCovstruct$S.SE["SMRV06","SMRV06"] + cCovstruct$S.SE["SMRV08","SMRV08"])/4,

  mean(cCovstruct$S.SE,na.rm=T),
  
  mean(diag(cCovstruct$S.SE),na.rm=T),
  
  (cCovstruct$S.SE["SMRV01","SMRV02"] + cCovstruct$S.SE["SMRV03","SMRV04"] + 
    cCovstruct$S.SE["SMRV05","SMRV06"] + cCovstruct$S.SE["SMRV07","SMRV08"])/4,
  
  mean(100*(cCovstruct$S.SE-ltCovstructS.SE)/ltCovstructS.SE,na.rm=T),
  mean(100*(cCovstruct$S.SE-diagCovstructS.SE)/diagCovstructS.SE,na.rm=T),
  mean(100*(cCovstruct$S.SE-offDiagCovstructS.SE)/offDiagCovstructS.SE,na.rm=T),
  
  sqrt(mean((cCovstruct$S.SE-ltCovstructS.SE)^2,na.rm=T)),
  sqrt(mean((cCovstruct$S.SE-diagCovstructS.SE)^2,na.rm=T)),
  sqrt(mean((cCovstruct$S.SE-offDiagCovstructS.SE)^2,na.rm=T)),
  
  #CV
  (S.CV["SMRV01","SMRV01"] + S.CV["SMRV03","SMRV03"] + 
    S.CV["SMRV05","SMRV05"] + S.CV["SMRV07","SMRV07"])/4,
  
   (S.CV["SMRV02","SMRV02"] + S.CV["SMRV04","SMRV04"] + 
    S.CV["SMRV06","SMRV06"] + S.CV["SMRV08","SMRV08"])/4,
  
  mean(S.CV,na.rm=T),
  
  mean(diag(S.CV),na.rm=T),
  
  (S.CV["SMRV01","SMRV02"] + S.CV["SMRV03","SMRV04"] + 
    S.CV["SMRV05","SMRV06"] + S.CV["SMRV07","SMRV08"])/4,
  
  mean(100*(S.CV-ltCovstructS.CV)/ltCovstructS.CV,na.rm=T),
  mean(100*(S.CV-diagCovstructS.CV)/diagCovstructS.CV,na.rm=T),
  mean(100*(S.CV-offDiagCovstructS.CV)/offDiagCovstructS.CV,na.rm=T),
  
  sqrt(mean((S.CV-ltCovstructS.CV)^2,na.rm=T)),
  sqrt(mean((S.CV-diagCovstructS.CV)^2,na.rm=T)),
  sqrt(mean((S.CV-offDiagCovstructS.CV)^2,na.rm=T))
...
#GCTA REML
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS.SE)

#emulated standard LDSC
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS.SE)

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS.SE)

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS.SE)

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS.SE)

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS.SE)

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS.SE)

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS.SE)

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS.SE)

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesFull("gladp.sim.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS.SE)
...
createTableEntriesPairwise<-function(code,cCovstruct,simulatedTraitsTable,t1,t2){
  ...
  cCovstruct$S.SE[t1,t1],
  
  cCovstruct$S.SE[t2,t2],
  
  mean(cCovstruct$S.SE[c(t1,t2),c(t1,t2)],na.rm=T),
  
  mean(diag(cCovstruct$S.SE[c(t1,t2),c(t1,t2)]),na.rm=T),
  
  cCovstruct$S.SE[t1,t2],
  
  mean(100*(cCovstruct$S.SE[c(t1,t2),c(t1,t2)]-ltCovstructS.SE[c(t1,t2),c(t1,t2)])/ltCovstructS.SE[c(t1,t2),c(t1,t2)],na.rm=T),
  mean(100*(cCovstruct$S.SE[c(t1,t2),c(t1,t2)]-diagCovstructS.SE[c(t1,t2),c(t1,t2)])/diagCovstructS.SE[c(t1,t2),c(t1,t2)],na.rm=T),
  mean(100*(cCovstruct$S.SE[c(t1,t2),c(t1,t2)]-offDiagCovstructS.SE[c(t1,t2),c(t1,t2)])/offDiagCovstructS.SE[c(t1,t2),c(t1,t2)],na.rm=T),
  
  sqrt(mean((cCovstruct$S.SE[c(t1,t2),c(t1,t2)]-p$mvLD$covstruct.GCTA$S.SE[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  sqrt(mean((cCovstruct$S.SE[c(t1,t2),c(t1,t2)]-diagCovstructS.SE[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  sqrt(mean((cCovstruct$S.SE[c(t1,t2),c(t1,t2)]-offDiagCovstructS.SE[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  ...
  mean(S.CV[c(t1,t2),c(t1,t2)],na.rm=T),
  
  mean(diag(S.CV[c(t1,t2),c(t1,t2)]),na.rm=T),
  
  S.CV[t1,t2],
  
  mean(100*(S.CV[c(t1,t2),c(t1,t2)]-ltCovstructS.CV[c(t1,t2),c(t1,t2)])/ltCovstructS.CV[c(t1,t2),c(t1,t2)],na.rm=T),
  mean(100*(S.CV[c(t1,t2),c(t1,t2)]-diagCovstructS.CV[c(t1,t2),c(t1,t2)])/diagCovstructS.CV[c(t1,t2),c(t1,t2)],na.rm=T),
  mean(100*(S.CV[c(t1,t2),c(t1,t2)]-offDiagCovstructS.CV[c(t1,t2),c(t1,t2)])/offDiagCovstructS.CV[c(t1,t2),c(t1,t2)],na.rm=T),
  
  sqrt(mean((S.CV[c(t1,t2),c(t1,t2)]-ltCovstructS.CV[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  sqrt(mean((S.CV[c(t1,t2),c(t1,t2)]-diagCovstructS.CV[c(t1,t2),c(t1,t2)])^2,na.rm=T)),
  sqrt(mean((S.CV[c(t1,t2),c(t1,t2)]-offDiagCovstructS.CV[c(t1,t2),c(t1,t2)])^2,na.rm=T))
...
#GCTA REML
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#emulated standard LDSC
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim100.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV01","SMRV02")

#**75% OVERLAP**

#GCTA REML
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#emulated standard LDSC
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim75.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV03","SMRV04")

#**50% OVERLAP**

#GCTA REML
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#emulated standard LDSC
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim50.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV05","SMRV06")


#**25% OVERLAP**

#GCTA REML
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.gcta",p$mvLD$covstruct.GCTA,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#emulated standard LDSC
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.gsem",p$mvLD$covstruct.mvLDSC.GSEMemulation.1kg.maf0_01.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ VBCS, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.vbcs.v.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ VBCS, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.vbcs.r.now",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, number of variants blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.jn.v.now",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, recombination distance blocks, no weighting extensions
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.jn.r.now",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ VBCS, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.vbcs.v",p$mvLD$covstruct.mvLDSC.1kg.vbcs.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ VBCS, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.vbcs.r",p$mvLD$covstruct.mvLDSC.1kg.vbcs.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, number of variants blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.jn.v",p$mvLD$covstruct.mvLDSC.1kg.jn.varblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")

#LDSC++ Block Jackknife, recombination distance blocks
simulatedTraitsTableS.SE<-createTableEntriesPairwise("gladp.sim25.jn.r",p$mvLD$covstruct.mvLDSC.1kg.jn.rblock.altcw.sim,simulatedTraitsTableS.SE,"SMRV07","SMRV08")


```

# Create simulation results table and plot for main manuscript
The table for real world data is created in the rg overview.
```{r simulation results stable, fig.width=9, fig.height=6, out.width="1600px", out.height="1000px", eval=FALSE, purl=FALSE}

simulationResTableLDruns<- c(
  "gladp.sim.gcta",
  "gladp.sim.gsem",
# "gladp.sim.vbcs.v.now",
# "gladp.sim.vbcs.r.now",
# "gladp.sim.jn.v.now",
# "gladp.sim.jn.r.now",
"gladp.sim.vbcs.v",
"gladp.sim.vbcs.r",
...
simulationResultsTableDf<-rbind(t(simulatedTraitsTableS[simulationResTableLDruns,]),t(simulatedTraitsTableS.SE[simulationResTableLDruns,]))

#rownames(simulationResultsTableDf)
selRows<-c(
  "Avg. SIM h2",
  "Avg. SIM1 & SIM2 covG",
  "RMSE(S)",
  ...
  "RMSE(off diag S)",
  # "Avg. SIM h2 S.E.",
  # "Avg. SIM1 & SIM2 covG S.E.",
  # "RMSE(S S.E.)",
  # "RMSE(diag S S.E.)",
  # "RMSE(off diag S S.E.)",
  "Avg. SIM h2 CV",
  "Avg. SIM1 & SIM2 covG CV",
  "RMSE(CV)",
  "RMSE(diag S CV)",
  "RMSE(off diag S CV)"
)
#selRows %in% rownames(simulationResultsTableDf)
simulationResultsTableDf<-simulationResultsTableDf[selRows,]

rownames(simulationResultsTableDf)[rownames(simulationResultsTableDf)=="Avg. SIM h2"]<-"Avg. SIM h²"
# rownames(simulationResultsTableDf)[rownames(simulationResultsTableDf)=="Avg. SIM h2 S.E."]<-"Avg. SIM h² S.E."
rownames(simulationResultsTableDf)[rownames(simulationResultsTableDf)=="Avg. SIM h2 CV"]<-"Avg. SIM h² CV"
rownames(simulationResultsTableDf)[rownames(simulationResultsTableDf)=="RMSE(CV)"]<-"RMSE(S CV)"


as.data.frame(simulationResultsTableDf) %>% 
  ...
  tab_header(
    ...
    gladp.sim.gcta = "GCTA REML",
    gladp.sim.gsem = "LDSC (Genomic SEM, emulated in LDSC++)",
gladp.sim.vbcs.v="LDSC++ varible block count sampling, blocks defined by no. variants",
gladp.sim.vbcs.r="LDSC++ varible block count sampling, blocks defined by recombination distance",
gladp.sim.jn.v="LDSC++ block jackknife resampling, blocks defined by no. variants",
gladp.sim.jn.r="LDSC++ block jackknife resampling, blocks defined by recombination distance"
  ) %>%
  fmt_number(
    decimals = 4
  ) |>
   tab_row_group(
    #label = "Heritability and covG standard error",
    label = "Heritability and covG CV",
    rows = 6:10
  ) %>%
 tab_row_group(
    label = "Heritability and covG",
    rows = 1:5
  ) |>
  tab_style(
    style = cell_text(size = px(16),weight = "bold"),
...
#plots
plotTable<- as.data.frame(t(simulationResultsTableDf))
plotTable$methodName <- unlist(list(
  "GCTA-GREML",
  "LDSC (Genomic SEM)",
  "LDSC++ varible block count sampling,\nno. variants blocks",
  "LDSC++ varible block count sampling,\nrecombination distance blocks",
  "LDSC++ block jackknife resampling,\nno. variants blocks",
  "LDSC++ block jackknife resampling,\nrecombination distance blocks"
  ))
plotTable$code<-rownames(plotTable)
plotTable$`Method`<-as.factor(plotTable$methodName)

plotTable$`Avg. SIM covG`<-(plotTable$`Avg. SIM h²`+plotTable$`Avg. SIM1 & SIM2 covG`)/2
plotTable$`Avg. SIM covG CV`<-(plotTable$`Avg. SIM h² CV`+plotTable$`Avg. SIM1 & SIM2 covG CV`)/2

plotTable$`Avg. covG difference (%) rel. REML`<-100*(plotTable$`Avg. SIM covG`-plotTable["gladp.sim.gcta",]$`Avg. SIM covG`)/plotTable["gladp.sim.gcta",]$`Avg. SIM covG`
plotTable$`Avg. covG CV difference (%) rel. REML`<-100*(plotTable$`Avg. SIM covG CV`-plotTable["gladp.sim.gcta",]$`Avg. SIM covG CV`)/plotTable["gladp.sim.gcta",]$`Avg. SIM covG CV`

plotTable$`Avg. h² difference (%) rel. REML`<-100*(plotTable$`Avg. SIM h²`-plotTable["gladp.sim.gcta",]$`Avg. SIM h²`)/plotTable["gladp.sim.gcta",]$`Avg. SIM h²`
plotTable$`Avg. h² CV difference (%) rel. REML`<-100*(plotTable$`Avg. SIM h² CV`-plotTable["gladp.sim.gcta",]$`Avg. SIM h² CV`)/plotTable["gladp.sim.gcta",]$`Avg. SIM h² CV`

plotTable$`Avg. covG difference (%) rel. LDSC`<-100*(plotTable$`Avg. SIM covG`-plotTable["gladp.sim.gsem",]$`Avg. SIM covG`)/plotTable["gladp.sim.gsem",]$`Avg. SIM covG`
plotTable$`Avg. covG CV difference (%) rel. LDSC`<-100*(plotTable$`Avg. SIM covG CV`-plotTable["gladp.sim.gsem",]$`Avg. SIM covG CV`)/plotTable["gladp.sim.gsem",]$`Avg. SIM covG CV`

plotTable$`Avg. h² difference (%) rel. LDSC`<-100*(plotTable$`Avg. SIM h²`-plotTable["gladp.sim.gsem",]$`Avg. SIM h²`)/plotTable["gladp.sim.gsem",]$`Avg. SIM h²`
plotTable$`Avg. h² CV difference (%) rel. LDSC`<-100*(plotTable$`Avg. SIM h² CV`-plotTable["gladp.sim.gsem",]$`Avg. SIM h² CV`)/plotTable["gladp.sim.gsem",]$`Avg. SIM h² CV`

plotTable_h2<-plotTable[,c("Avg. SIM h²","Avg. SIM h² CV","RMSE(diag S)","RMSE(diag S CV)","Method")]
plotTable_covG<-plotTable[,c("Avg. SIM1 & SIM2 covG","Avg. SIM1 & SIM2 covG CV","RMSE(off diag S)","RMSE(off diag S CV)","Method")]
plotTable_covG_tot<-plotTable[,c("Avg. SIM covG","Avg. SIM covG CV","RMSE(S)","RMSE(S CV)","Method")]
colnames(plotTable_h2)<-colnames(plotTable_covG)<-colnames(plotTable_covG_tot)<-c("covG","cv","RMSE(covG)","RMSE(covG CV)","Method")
plotTable_h2$group<-"Diagonal\ncovG (h²)"
plotTable_covG$group<-"Off-diagonal\ncovG"
plotTable_covG_tot$group<-"Total covG"
plotTable2<-rbind(plotTable_h2,plotTable_covG,plotTable_covG_tot)

#plotTable$cMean<-plotTable$`Avg. SIM h²`
#plotTable$cCV<-plotTable$`Avg. SIM h² CV`
plotTable2$upperCV<-plotTable2$covG+plotTable2$cv
plotTable2$lowerCV<-plotTable2$covG-plotTable2$cv

#cvScaleScalingFactor<-mean(plotTable2$covG) #what should this be????

h2plot <- plotTable2 %>% ggplot(aes(x=Method, y=covG)) + #"Method"
  geom_point(show.legend = F) +
  geom_errorbar(aes(ymin=lowerCV, ymax=upperCV), show.legend = F) +
  geom_line(stat = "identity", aes(x=Method, y=(`RMSE(covG)` - 0.04)*10, colour="red",fill="red", group = group), show.legend = F) + #aes(x=Method, y = `RMSE(covG)`, color="red") stat = "identity"
  geom_line(stat = "identity", aes(x=Method, y=(`RMSE(covG CV)` - 0.04)*10, colour="blue", fill="blue", group = group), show.legend = F) +
  scale_color_manual(values=c("blue", "red"))+
  scale_y_continuous(
    "covG and covG CV estimates on the same scale",
    sec.axis = sec_axis(~ (. + 0.4)/10, name = "RMSE (covG or covG CV)")
  ) +
  ...
  scale_x_discrete(guide = guide_axis(angle = 45)) + #n.dodge = 0
  facet_grid(rows = vars(group))

h2plot

ggsave(filename=file.path(p$folderpath.plots, paste0("method.covG.CV.png")), plot = h2plot, width = 297, height = 210, units = "mm", dpi = 320, scale = 0.8)
...
# edit new_metabolites for Xinyue
new_metabolites$file_name<-str_replace(string = new_metabolites$file_name, "\\.tsv","\\.gz")

#munge example
# 
#  mungeResults <- supermunge(
#             filePaths = c(
#               "original/MDD_BIP/MDD2018_ex23andMe_rezip.gz",
#               "original/MDD_BIP/MDD_MHQ_METACARPA_INFO6_A5_NTOT_no23andMe_noUKBB.rezip.gz"
#             ),
#             #refFilePath = '/scratch/prj/gwas_sumstats/variant_lists/reference.1000G.maf.0.005.txt.gz',
#             traitNames = c(
#               "DEPR_cole_PGC_UKBB_ex23andMe",
#               "DEPR_cole_MHQ_ex23andMe_exUKBB"
#             ),
#             filter.info = 0.6,
#             filter.maf = 0.001,
#             process = F,
#             #N = [list of N values],
#             pathDirOutput = "cleaned"
#               )
#     
 # mungeResults <- supermunge(
 #            filePaths = c(
