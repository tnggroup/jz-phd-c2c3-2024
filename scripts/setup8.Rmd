...
p$folderpath.data.mvLDSC.ld.1kg <- file.path(p$folderpath.data,"ld_scores","hc1kgp3.b38.eur.l2")
...
##Gene mapping
p$filepath.geneMapping<-file.path(p$folderpath.data,"gene_mapping","NCBI37.3.gene.locUPDATETHIS")
...
p$printCorr <- function(corr, SE=NULL, filename, addrect = 4, is.corr = T, number.cex = 1.5, number.digits=2){
...
p$sumstats[nrow(p$sumstats)+1,c("code","name", "n_case","n_control","n_total","phenotype_id","reference_id","reference_doi")]=list(
  code="GWISM",
  name="GWIS of BMI",
  n_case=3612229, #this is the N of the height component
  n_control=NA_integer_,
  n_total=3612229,
  phenotype_id=NA_integer_,
  reference_id=NA_integer_,
...
if(p$host=="local"){
  ...
  p$sumstats["HEIG09",]$cleanedpath<-"/scratch/prj/gwas_sumstats/original/GIANT_HEIGHT_YENGO_2022_GWAS_SUMMARY_STATS_EUR.gz"
}
...
p$sumstats$name.nice[which(p$sumstats$code=="BODY11")]="BMI"
...
p$sumstats.sel.gwis.code<-c("GWISM","GWISI","BODY11")

p$sumstats.sel.code <- p$sumstats.sel.ssimp.code


#all traits
#p$sumstats.sel.code<-c("ADHD05","ALCD03","ANOR02","ANXI03","ANXI04","AUTI07","BODY11","BIPO02", "BVOL01","DEPR05","DEPR5B","DEPR09","EDUC03","HEAL01","HEIG09","INCO03","INSO03","INSO02","NEUR02", "PTSD04","RISK02","RISK03","SCHI04","SUBJ01","TIRE01","WEIG04F","WEIG04M")
...
p$sumstats.sel.gwis<-p$sumstats[which(p$sumstats$code %in% p$sumstats.sel.gwis.code),]
...
if(p$clOptions$task=="munge" & !all(file.exists(p$sumstats.sel$mungedpath))){
  ...
  #test
  #p$clOptions$task_argument<-"WEIG04F"
  ...
  p$munge$results <- supermunge(
            ...
            #invertEffectDirectionOn = c("ANXI04") #ANXI4 has an inverted effect direction for some reason - SHOULD BE FIXED NOW WITH IMPROVED ALLELE HANDLING!
              ) 
...
if(p$clOptions$task=="munge2"){
  ...
  p$munge$results<-supermunge(
    ...
    info.filter = 0.6,
    maf.filter = 0.01,
    imputeAdjustN = T, #TEMPORARY - Only for traits imputed before updated supermunge - not for DEPR5B
    N = p$sumstats.sel$n_total,
    region.filter_df = p$highld_b38,
    chr.filter = list(1,2,6,7,9,11),
...
if(p$clOptions$task=="gwis"){
  ...
  p$performGWIS <- function(nCode,pathGwasH,patghGwasWM,pathGwasWF){
    ...
    gwasW<-copy(gwasWM)
    gwasW[gwasWF, on="SNP", c("BETA","SE","N","FRQ"):=list((W*BETA+i.W*i.BETA)/(W+i.W), sqrt(1/(W+i.W)), N+i.N, shru::clipValues((FRQ/N+i.FRQ/i.N)/(1/N+1/i.N),min = 1/(10*N), max = 1))][,W:=1/(SE^2)]
    ...
    gwasBMI <- copy(gwasH)
    gwasBMI[gwasW,on=c("SNP"),c("BETA.W","SE.W","BETA.H","SE.H","N","FRQ","Z.H","Z.W","N.H","N.W"):=list(i.BETA, i.SE, BETA, SE, (N+i.N)/2, shru::clipValues( (FRQ/N+i.FRQ/i.N)/(1/N+1/i.N),min = 1/(10*N), max = 1), Z, i.Z, N, i.N)]
    
    gwasBMI<-gwasBMI[is.finite(BETA.W) & is.finite(BETA.H),]
    
    #different types of Z
    gwasBMI[,Z.BMI:=Z.W/(Z.H^2)]
    gwasBMI[,Z.WEIGHTED:=(Z.W/(N.W*FRQ)+Z.H/(N.H*FRQ))/(1/(N.W*FRQ)+1/(N.H*FRQ))]
    ...
    #GWIS
    gwasBMI[,c("BETA"):=list(
      (2*FRQ*(1-FRQ)/(2*FRQ*(1-FRQ)+FRQ^2))*((I.W+BETA.W*eval(..sdWeightKg))/((I.H+BETA.H*eval(..sdHeightM))^2) - I.BMI) + (1/2)*((FRQ^2)/(2*FRQ*(1-FRQ)+FRQ^2))*((I.W+2*BETA.W*eval(..sdWeightKg))/((I.H+2*BETA.H*eval(..sdHeightM))^2) - I.BMI)
    )]
    
    
    #SE of BMI - a ratio of random variables - no closed form solution for the variance
    gwasBMI[,c("SE.W.S","SE.H2.S","E.H2.S"):=list(
      sqrt((SE.W*eval(..sdWeightKg))^2), #scaled standard error
      sqrt(2 + 2*((I.H/(SE.H*eval(..sdHeightM)))^2)), #scaled standard error
      1 + (I.H/(SE.H*eval(..sdHeightM)))^2 #scaled estimated value - THIS IS THE NEW MEAN OF H2!!!
    )]
    ...
    gwasBMI[,c("SE.DERIVED","E.BMI","SE.BMI.Z.DERIVED","SE.WEIGHTED"):=list(
      sqrt(
        ((I.W^2)/(E.H2.S^2)) * ((SE.W.S^2)/(I.W^2) - 2*(0.5*SE.W.S*SE.H2.S)/(I.W*E.H2.S) + (SE.H2.S^2)/(E.H2.S^2))
        ), #assume a 0.5 correlation. the approximation of a ratio random variables using taylor series.
      I.W/E.H2.S - 0.5*SE.W.S*SE.H2.S/(I.W^2) + (SE.H2.S^2)*I.W/(I.W^3),
      abs(BETA/Z.BMI),
      abs(BETA/Z.WEIGHTED)
    )]
    ...
    gwasBMI[,SE:=SE.WEIGHTED][,c("Z"):=list(BETA/SE)][,P:=2*pnorm(q = abs(Z), mean = 0, sd = 1, lower.tail = F)]
    
    gwasBMI <- gwasBMI[is.finite(BETA.W) & is.finite(P), c("SNP","A1","A2","CHR","BP","FRQ","BETA","SE","Z","P","N")]
    
    cat("Number of rows in GWIS: ", nrow(gwasBMI))
...
p$filepath.mvLD.gwis<-file.path(p$folderpath.workingDirectory,paste0("mvLD.gwis.",p$setup.code,".Rds"))
...
if (file.exists(p$filepath.mvLD.gwis)) {
  print("Using existing covariance structures from previous LD computations (BMI GWAS)")
  p$mvLD.gwis<-readRDS(file=p$filepath.mvLD.gwis)
} else {
  p$mvLD.gwis<-c()
}
...
if(p$clOptions$task=="mvLD" || (!file.exists(p$filepath.mvLD) & p$clOptions$task=="0"))
{
  ...
  #   filter.simpute.fraction = 0.5, #maximise this for demo
  #   filter.chisq.min = 0.01579077, #qchisq(p = 0.1,df = 1)
  #   preweight.SINFO = T,
  #   filter.region.df = p$highld_b38,
  #   N = p$sumstats.sel$n_total,
  ...
  #   filter.sinfo = 0.1,
  #   filter.sinfo.imputed = 0.95,
  #   filter.simpute.fraction = 0.05, #5% as heavily filtered data
  #   filter.chisq.min = 0.01579077, #qchisq(p = 0.1,df = 1)
  #   preweight.SINFO = T,
  #   filter.region.df = p$highld_b38,
  #   N = p$sumstats.sel$n_total,
...
p$sumstats.sel$h2.liability_mvLDSC.1kg.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.small$S[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.small$S.SE[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.liability_mvLDSC.1kg.ldimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.500$S[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.ldimp<-diag(p$mvLD$covstruct.mvLDSC.1kg.500$S.SE[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.liability_mvLDSC.1kg.ldimp.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.500.small$S[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.ldimp.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.500.small$S.SE[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.liability_mvLDSC.1kg.ssimp.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$S[p$sumstats.sel$code,p$sumstats.sel$code])
p$sumstats.sel$h2.se.liability_mvLDSC.1kg.ssimp.small<-diag(p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$S.SE[p$sumstats.sel$code,p$sumstats.sel$code])
...
p$sumstats.sel$variants_ldimp.small<-p$mvLD$covstruct.mvLDSC.1kg.500.small$impstats$m.imputed
p$sumstats.sel$variants_ssimp.small<-p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$impstats$m.imputed

p$sumstats.sel$variants_raw_merged<-p$mvLD$covstruct.mvLDSC.1kg$impstats$m.merged
p$sumstats.sel$variants_raw_merged.small<-p$mvLD$covstruct.mvLDSC.1kg.small$impstats$m.merged
p$sumstats.sel$variants_ldimp_merged<-p$mvLD$covstruct.mvLDSC.1kg.500$impstats$m.merged.imputed
p$sumstats.sel$variants_ldimp_merged.small<-p$mvLD$covstruct.mvLDSC.1kg.500.small$impstats$m.merged.imputed
p$sumstats.sel$variants_ssimp_merged.small<-p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$impstats$m.merged.imputed

p$sumstats.sel$variants_nonimputed_mean_cell<-p$mvLD$covstruct.mvLDSC.1kg.500$impstats$m.cell.mean.nonimputed
p$sumstats.sel$variants_nonimputed_mean_cell.small<-p$mvLD$covstruct.mvLDSC.1kg.500.small$impstats$m.cell.mean.nonimputed
p$sumstats.sel$variants_ldimp_mean_cell<-p$mvLD$covstruct.mvLDSC.1kg.500$impstats$m.cell.mean.imputed
p$sumstats.sel$variants_ldimp_mean_cell.small<-p$mvLD$covstruct.mvLDSC.1kg.500.small$impstats$m.cell.mean.imputed
p$sumstats.sel$variants_ssimp_mean_cell.small<-p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$impstats$m.cell.mean.imputed

p$sumstats.sel$variants_ldimp_mean_cell_full<-p$mvLD$covstruct.mvLDSC.1kg.500$impstats$m.cell.mean.imputed.full
p$sumstats.sel$variants_ldimp_mean_cell_full.small<-p$mvLD$covstruct.mvLDSC.1kg.500.small$impstats$m.cell.mean.imputed.full
p$sumstats.sel$variants_ssimp_mean_cell_full.small<-p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$impstats$m.cell.mean.imputed.full

p$sumstats.sel$variants_ldimp_mean_cell_partial<-p$mvLD$covstruct.mvLDSC.1kg.500$impstats$m.cell.mean.imputed.partial
p$sumstats.sel$variants_ldimp_mean_cell_partial.small<-p$mvLD$covstruct.mvLDSC.1kg.500.small$impstats$m.cell.mean.imputed.partial
p$sumstats.sel$variants_ssimp_mean_cell_partial.small<-p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$impstats$m.cell.mean.imputed.partial
...
p$sumstats.sel$chisquare_mean_ssimp.small<-p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$impstats$meanChiSquare.imputed


#add per trait mean differences in standard error
...
for(cTrait in rownames(p$sumstats.sel)){
  ...
  p$sumstats.sel[cTrait,c("mean.S_Stand.SE.deltaimpnoimp.ssimp")]<-mean(p$mvLD$S_Stand.SE.deltaimpnoimp.ssimp[cTrait,],na.rm=T)
  p$sumstats.sel[cTrait,c("mean.S_Stand.SE.deltaimpnoimp.ssimp.rel")]<-mean(p$mvLD$S_Stand.SE.deltaimpnoimp.ssimp.rel[cTrait,],na.rm=T)
}
...
# p$mvLD$covstruct.mvLDSC$S_Stand.forPlot<-p$mvLD$covstruct.mvLDSC$S_Stand
# rownames(p$mvLD$covstruct.mvLDSC$S_Stand.forPlot)<-p$sumstats.sel$name.nice.and_code
# colnames(p$mvLD$covstruct.mvLDSC$S_Stand.forPlot)<-p$sumstats.sel$code
#
...
heatmaply(
    clipValues(p$mvLD$covstruct.mvLDSC.1kg$S_Stand,-1,1),
...
heatmaply(
    p$mvLD$covstruct.mvLDSC.1kg.500$S,
...
heatmaply(
    clipValues(p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand,-1,1),
    cellnote = round(clipValues(p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand,-1,1),digits = 2),
    cellnote_size = 16,
    cellnote_textposition = "middle center",
    show_dendrogram = c(T,F),
    xlab = "Trait GWAS",
...
heatmaply(
    clipValues(p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$S_Stand,-1,1),
    cellnote = round(clipValues(p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$S_Stand,-1,1),digits = 2),
    cellnote_size = 16,
    cellnote_textposition = "middle center",
    show_dendrogram = c(T,F),
    xlab = "Trait GWAS",
    ...
    file=file.path(p$folderpath.plots,paste0("rg_cluster.indicator.ssimp.png")),
    width = 1200, height = 1200,
    hclust_method = "ward.D2"
    #scale = "row"
  )
...
p$printCorr(
  ...
  SE = p$mvLD$covstruct.mvLDSC.1kg$S.SE,
...
#   corr = clipValues(p$mvLD$covstruct.mvLDSC.1kg$S_Stand,-1,1),
#   SE = p$mvLD$covstruct.mvLDSC.1kg$S_Stand.SE,
#   filename = file.path(p$folderpath.plots,"rg.indicator.10.png"),
#   is.corr = T,
#   addrect = NULL
#   )
...
p$printCorr(
  ...
  SE = p$mvLD$covstruct.mvLDSC.1kg.500$S.SE,
...
p$printCorr(
  ...
  SE = p$mvLD$covstruct.mvLDSC.1kg.500$S_Stand.SE,
...
p$printCorr(
  ...
  SE = p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$S.SE,
...
p$printCorr(
  ...
  SE = p$mvLD$covstruct.mvLDSC.1kg.ssimp.small$S_Stand.SE,
...
#chisquare relationships
p$sumstats.sel.table <- p$sumstats.sel[,c("name.nice.and_code","chisquare_mean","mean.S_Stand.SE.deltaimpnoimp.500","mean.S_Stand.SE.deltaimpnoimp.500.rel","mean.S_Stand.SE.deltaimpnoimp.ssimp","mean.S_Stand.SE.deltaimpnoimp.ssimp.rel")]


ggplot(data = p$sumstats.sel.table, aes(x = chisquare_mean, y = mean.S_Stand.SE.deltaimpnoimp.500.rel, color = name.nice.and_code, label = name.nice.and_code)) +
geom_point() +
geom_line() +
geom_text_repel()
...
p$plots.sumstats.sel.table<-p$sumstats.sel.table %>% 
  ...
  tab_header(
    title = "Trait heritabilities: panel comparison"
  ) %>% cols_label(
    name.nice.and_code  = "Trait",
...
#selected sumstats with heritabilities, small
p$sumstats.sel.table<-p$sumstats.sel[,c("name.nice.and_code",
                                        "chisquare_mean",
                                        "h2.liability_mvLDSC.hm3",
                                        "h2.se.liability_mvLDSC.hm3",
                                        ...
                                        "h2_orig.liability_mvLDSC.1kg.small",
                                        "h2_orig.se.liability_mvLDSC.1kg.small",
                                        "h2_orig.liability_mvLDSC.1kg.ldimp.small",
                                        "h2_orig.se.liability_mvLDSC.1kg.ldimp.small",
                                        "h2_orig.liability_mvLDSC.1kg.ssimp.small",
                                        "h2_orig.se.liability_mvLDSC.1kg.ssimp.small"
                                        )]
...
p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.table<-paste0(round( p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.small,digits = 3)," (",round(p$sumstats.sel.table$h2_orig.se.liability_mvLDSC.1kg.small,digits = 4),")")

p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.ldimp.table<-paste0(round( p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.ldimp.small,digits = 3)," (",round(p$sumstats.sel.table$h2_orig.se.liability_mvLDSC.1kg.ldimp.small,digits = 4),")")

p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.ssimp.table<-paste0(round( p$sumstats.sel.table$h2_orig.liability_mvLDSC.1kg.ssimp.small,digits = 3)," (",round(p$sumstats.sel.table$h2_orig.se.liability_mvLDSC.1kg.ssimp.small,digits = 4),")")


# p$sumstats.sel.table$log_nxm_mvldsc_1kg500_nohighld<- log(p$sumstats.sel.table$n_case_total*p$sumstats.sel.table$variants_mvldsc_1kg500_nohighld)
# 
# p$sumstats.sel.table$log_nxm_mvldsc_1kgssimp_nohighld<- log(p$sumstats.sel.table$n_case_total*p$sumstats.sel.table$variants_mvldsc_1kgssimp_nohighld)
...
p$plots.sumstats.sel.table<-p$sumstats.sel.table %>% 
  ...
  fmt_number(columns = vars(chisquare_mean), decimals = 2) %>%
  tab_header(
    title = "Trait heritabilities: restricted* method comparison"
  ) %>% cols_label(
    name.nice.and_code  = "Trait",
    chisquare_mean = html("Mean X<sup>2</sup>, merged"),
...
# Visualise imputation quality measurements
```{r visualise imputation quality measurements, fig.width=9, fig.height=6, out.width="1600px", out.height="1000px", eval=FALSE, purl=FALSE}
...
for(iTrait in 1:nrow(p$sumstats.sel)){
  ...
  cSumstats[IMPUTED==F & is.na(LDIMP.L2.SUM),GROUP:=1]
  cSumstats[IMPUTED==F & is.finite(LDIMP.L2.SUM),GROUP:=2]
  ...
  cSumstatsPlot <- cSumstats[is.finite(ZZ) & ZZ>0.01579077 & ZZ< 50,] #CHR == 1 #c("SNP","CHR", "IMPUTED","NAL","Z", "ZZ","ZZ.I","SINFO","LDIMP.L2.SUM","LDIMP.K","GROUP",'d.z','d.beta','d.se','d.z.abs','d.beta.abs','d.se.abs')
  cSumstatsPlot$index<-1:nrow(cSumstatsPlot)
  cSumstatsPlot$bucket<-ntile(cSumstatsPlot$index,n = nrow(cSumstatsPlot)/50)
  cSumstatsPlot[, c('iLDIMP.L2.SUM','iLDIMP.K') := list(mean(LDIMP.L2.SUM,na.rm=T), mean(LDIMP.K,na.rm=T)), by = bucket]
  cSumstatsPlot[GROUP==1,c('LDIMP.L2.SUM','LDIMP.K'):=list(iLDIMP.L2.SUM,iLDIMP.K)] #non-imputed, non-validated
...
#   (ggplot(cSumstatsPlot[LD_IMP<50,], aes(x=ZZ,y=LD_IMP)) +
