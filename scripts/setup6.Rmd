...
project$CFA$correlation<-c("COR") #ORT, OBL, and COR
project$CFA$estimator=c("ML")
...
if(project$clOptions$task=="munge"){
  ...
  project$supermunge.results <- supermunge(filePaths = project$munge$filesToUse,
  ...
  for(iFactorConfiguration in 1:length(project$CFA$nFactors)) {
    ...
    for(nSessioPattern in 1:sessionPatternLength){
      ...
      for(iCorrelationConfiguration in 1:length(project$CFA$correlation)){
        ...
        # #avoid correlated factors if above 7 factors
        ...
        # 
        # #avoid orthogonal factors if below 12 factors
        # if(project$CFA$nFactors[iFactorConfiguration]<12 & cCorrelation=="ORT") next
        ...
        for(iParameterEstimator in 1:length(project$CFA$estimator)){
          ...
          isIdentified <- (2*sum(cIndicatorLoadings) + factorial(nVariables)/(factorial(2)*factorial(nVariables-2))) <= nUniqueCovariances #real condition
          project$CFA$models[nModel,c("identified")]<-isIdentified
           isIdentified <- (2*sum(cIndicatorLoadings) + (factorial(nVariables)/(factorial(2)*factorial(nVariables-2)))/2) <= nUniqueCovariances #relax this condition in case my calculation is wrong
...
project$CFA$models.selected<-project$CFA$models[c("M25_4_27.COR.ML","M25_7_195.COR.ML","M25_10_176.OBL.ML"),] #this is a manual setting after choosing the best fitting models from the previous results 
