...
if(file.exists(project$filepath.lfgwas.sumstats)) {
  ...
  project$supermunge.results<-shru::supermunge(filePaths = project$sumstats.sel$mungedpath, refFilePath = project$filepath.SNPReference.1kg, traitNames = project$sumstats.sel$code, produceVariantTable = T, process = F, writeOutput = F)
...
if(project$clOptions$task=="nmeta" & !is.null(project$lfGWAS$sumstats)){
  
  project$lfGWAS$varmet<-c()
  colBeta<-colnames(project$lfGWAS$sumstats)[grep("^BETA\\.", ignore.case = TRUE,colnames(project$lfGWAS$sumstats))]
  colSE<-colnames(project$lfGWAS$sumstats)[grep("^SE\\.", ignore.case = TRUE,colnames(project$lfGWAS$sumstats))]
  ...
  diag(I)<-ifelse(diag(I)<= 1, 1, diag(I))
  I_diag<-diag(I)
  ...
  for (iCol in 1:ncol(BETASNP)){
    ...
    set(x = BETASNP, j = col, value = BETASNP[[col]]/I_diag[[iCol]])
    #BETASNP[, (col):=(..col)]
    #BETASNP[, (col):=(col)(I_diag[[iCol]])]
  }
  
  #set(x = BETASNP, j = colnames(BETASNP), value = BETASNP/t(I_diag))
  
  BETASNP.mean<-rowMeans(BETASNP,na.rm = T)
  BETASNP.sdiff<-BETASNP-BETASNP.mean
  SESNP<-project$lfGWAS$sumstats[,..colSE]
  VARSNP<-project$lfGWAS$sumstats$MAF*(1-project$lfGWAS$sumstats$MAF)
  #project$lfGWAS$sumstats[,eval("VARSNP"):=MAF*(1-MAF)]
  ...
  for(iModel in 1:nrow(project$CFA$models.selected)){
    ...
    cat("\nModel:  ",cModel$code)
    modelGWAS<-project$lfGWAS$sumstats[,c("SNP")]
    
    for(iFactor in 1:cModel$nFactors){
      #test
      #iFactor<-1
      cat("\nFactor:  ",iFactor)
      factorString.Z<-paste0("F",iFactor,".Z")
      factorString.SE<-paste0("F",iFactor,".SE")
      factorString.P<-paste0("F",iFactor,".P")
      W<-cModel$parsedGsemResults[[1]][[1]]$relativeVarianceExplainedPerFactor[,iFactor] #only weighted by the factor loadings here
      W2<-W^2
      k<-sum(!is.na(W))
      C<-sum(W,na.rm = T)-sum(W2,na.rm = T)/sum(W,na.rm = T)
      
      for(iSNP in 1:nrow(BETASNP)){
        #test iSNP<-1L
        if(iSNP %% 100000 == 0) cat("\n#SNP ",iSNP)
        ...
        for (x in 1:nrow(S)) {
          for(y in 1:ncol(S)){
            if (x != y)
              V_SNP[[x,y]]<-(SESNP[[iSNP,y]]*SESNP[[iSNP,x]]*I[[x,y]]*I[[x,x]]*I[[y,y]]*VARSNP[[iSNP]]^2)                else if (x == y)
              V_SNP[[x,x]]<-(SESNP[[iSNP,x]]*I[[x,x]]*VARSNP[[iSNP]])^2
          }
        }
        
        #this is slightly modified; scaled the expected variance to the absolute mean effect
        T2<-max( c(
          (sum(W*(BETASNP.sdiff[iSNP,]^2), na.rm = T)-(k-1)*abs(BETASNP.mean[[iSNP]]))/(C*abs(BETASNP.mean[[iSNP]])),
          0
          ))
        
        
        V_SNP_FULL<-colSums(V_SNP,na.rm = T)
        W_FULL<-W/(V_SNP_FULL+T2)
        
        
        
        
        set(x = modelGWAS,i =iSNP, j = factorString.SE,
            value = sqrt(sum(V_SNP_FULL,na.rm = T))
            )
        set(x = modelGWAS,i =iSNP, j = factorString.Z,
            value = (sum(W_FULL*BETASNP[iSNP,],na.rm = T)/sum(W_FULL, na.rm = T))/modelGWAS[[iSNP,(factorString.SE)]]
            )
        set(x = modelGWAS,i =iSNP, j = factorString.P,
            value = pnorm(q = abs(modelGWAS[[iSNP,(factorString.Z)]]),mean = 0, sd = 1, lower.tail = F)
            )
    ...
    saveRDS(object = modelGWAS,file = file.path(project$folderpath.workingDirectory,paste0("nmeta.",cModel$code,".",project$setup.code,".Rds")))
  print("Naive metaanalysis of this model is done and the result has been saved to a file.")
...
for(iModel in 1:length(project$lfGWAS$gwas)){
  ...
  cModel<-project$CFA$models.selected[iModel,]
  for(iGwas in 1:length(project$lfGWAS$gwas[[iModel]])){
    #test
    #iGwas<-1
    project$lfGWAS$gwas.for.display <- project$lfGWAS$gwas[[iModel]][[iGwas]]
    ...
    cTable <- project$lfGWAS$gwas.for.display[order(project$lfGWAS$gwas.for.display$p),c("SNP","CHR","MAF","p")] %>% 
      slice_head(n = 200) %>%
      mutate(i=row_number()) %>%
      select(i,SNP,CHR,MAF,p) %>%
    gt() %>% 
    fmt_scientific(columns = vars(p), decimals = 4) %>%
    fmt_number(columns = vars(MAF), decimals = 4) %>%
    fmt_number(columns = vars(CHR), decimals = 0) %>%
    tab_header(
      title = paste0("Top variants for ",cModel$code," F",iGwas)
    ) %>%
    tab_style(
      style = cell_text(size = px(12)),
      locations = cells_column_labels(everything())       
    ...
    gtsave(data = cTable, filename = file.path(project$folderpath.plots,cModel$code,paste0("top.F",iGwas,".rtf")))
    ...
    qqmanGwas$P<- shru::clipValues(qqmanGwas$p,min = 10^(-100), max = NULL)
    qqmanGwas<-qqmanGwas[which(!is.na(qqmanGwas$P)),]
    qqmanGwas<-qqmanGwas[,c("SNP","BP","CHR","P")]
    
    
    #manhattan plot 2
    png(filename = file.path(project$folderpath.plots,cModel$code,paste0("man.F",iGwas,".png")), width = 800, height = 600)
    qqman::manhattan(x = qqmanGwas)
    dev.off()
    
    #qqman::qq(project$lfGWAS$gwas.for.display$Pval_Estimate)
    
    png(filename = file.path(project$folderpath.plots,cModel$code,paste0("qq.F",iGwas,".png")), width = 800, height = 600)
