...
if(project$clOptions$task=="nmeta"){
  ...
  project$lfGWAS$sumstats<-readRDS(file=project$filepath.lfgwas.sumstats_full)
  print("Read summary statistics for latent factor GWAS from file.")
  ...
  #sumstats QC
  ## effects
  rsum<-rowSums(abs(project$lfGWAS$sumstats[,..colBeta]))
  ...
  POPVARSNP<-project$lfGWAS$sumstats$MAF*(1-project$lfGWAS$sumstats$MAF)
  ...
  for(iModel in 1:nrow(project$CFA$models.selected)){
    ...
    for(iFactor in 1:cModel$nFactors){
      ...
      for(iSNP in 1:nrow(BETASNP)){
        #test
        #iSNP<-1L
        if(!iSNP %% 100 == 0) next  #for testing
        if(iSNP %% 100000 == 0) cat("\n#SNP ",iSNP, project$lfGWAS$sumstats[[iSNP,c("SNP")]],"\n")
        ...
        mCovar.sum<-colSums(mCovar,na.rm = T)
        ...
        W_FULL<-W/(abs(mCovar.sum)+T2)
        
        
        
        set(x = modelGWAS,i =iSNP, j = "BETA",
            value = sum(W_FULL*BETASNP[iSNP,],na.rm = T)/sum(W_FULL, na.rm = T)
            )
      ...
      modelGWAS[, P:=pnorm(q = abs((BETA/SE)),mean = 0, sd = 1, lower.tail = F)]
      
      saveRDS(object = modelGWAS,file = file.path(project$folderpath.workingDirectory,paste0("nmeta.",cModel$code,".F",iFactor,".",project$setup.code,".Rds")))
...
for(iModel in 1:length(project$CFA$models.selected)){
  ...
  for(iFactor in 1:cModel$nFactors){
    ...
    project$lfGWAS$gwas.for.display <- project$lfGWAS$gwas.for.display[,Z:=BETA/SE]
    ...
    cTable <- project$lfGWAS$gwas.for.display[order(project$lfGWAS$gwas.for.display$Z,decreasing = T),] %>% 
      slice_head(n = 300) %>%
      mutate(i=row_number()) %>%
      select(i,SNP,CHR,MAF,BETA,SE,Z,P,Q) %>%
    gt() %>% 
    fmt_scientific(columns = vars(P), decimals = 4) %>%
    fmt_number(columns = vars(MAF,Z,SE,Q), decimals = 4) %>%
    ...
    qqmanGwas$P<- shru::clipValues(qqmanGwas$P,min = 10^(-300), max = NULL)
    ...
    #manhattan plot 2
    png(filename = file.path(project$folderpath.plots,cModel$code,paste0("man.F",iFactor,".png")), width = 800, height = 600)
    qqman::manhattan(x = qqmanGwas)
    dev.off()
    
    #qqman::qq(project$lfGWAS$gwas.for.display$Pval_Estimate)
    
    png(filename = file.path(project$folderpath.plots,cModel$code,paste0("qq.F",iFactor,".png")), width = 800, height = 600)
