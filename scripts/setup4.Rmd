...
for(iSelected in 1:nrow(project$CFA$models.selected)){
  ...
  df[,c("~disturbance")]<-cModel$parsedGsemResults[[1]][[1]]$residualVariancesSTDGenotype.matrix #add disturbances
  ...
  xlab("Indicator") +
  theme_bw(base_size=20) #use a black-and white theme with set font size
...
for(iModel in 1:nrow(project$CFA$models.selected)){
  ...
  if(!is.null(project$lfGWAS$gwas[[cModel$code]])){
    ...
    patCoefSq<-patCoef^2
    disVars<-cModel$parsedGsemResults[[1]][[1]]$residualVariancesSTDGenotype.matrix
...
```{r munge and export latent factor GWAS, eval=FALSE, purl=FALSE}
