...
project$printCorr(
  corr = clipValues(project$mvLD$covstruct.mvLDSC.1kg$S_Stand[project$sumstats.sel$code,project$sumstats.sel$code],-1,1),
  SE = project$mvLD$covstruct.mvLDSC.1kg$S_Stand.SE[project$sumstats.sel$code,project$sumstats.sel$code],
  filename = file.path(project$folderpath.plots,"rg.1kg_ld.png"),
  is.corr = T
  )
...
for(iSelected in 1:nrow(project$CFA$models.selected)){
  ...
  df[,c("~disturbance")]<-cModel$parsedGsemResults[[1]][[1]]$residualVaraiancesSTDGenotype.matrix #add disturbances
  ...
  scale_fill_gradient2(name = "Loading", 
                       high = theme.color$contrastLight3, mid = "white", low = theme.color$contrastDark3, 
                       midpoint=0, guide=F) +
...
for(iModel in 1:nrow(project$CFA$models.selected)){
  ...
  inflationFactor<-1/cModel$parsedGsemResults[[1]][[1]]$modelFit$totalVarianceExplained[1]
  if(!is.null(project$lfGWAS$gwas[[cModel$code]])){
    for(iFactor in 1:cModel$nFactors){
      ...
      project$lfGWAS$gwas[[cModel$code]][[iFactor]]$Z_adj<-project$lfGWAS$gwas[[cModel$code]][[iFactor]]$Z_Estimate*inflationFactor
      ...
      project$lfGWAS$gwas[[cModel$code]][[iFactor]]$p_adj<-2*pnorm(q = project$lfGWAS$gwas[[cModel$code]][[iFactor]]$Z_adj, lower.tail = F)
...
for(iModel in 1:length(project$lfGWAS$gwas)){
  ...
  for(iGwas in 1:length(project$lfGWAS$gwas[[iModel]])){
    ...
    cTable <- project$lfGWAS$gwas.for.display[order(project$lfGWAS$gwas.for.display$Pval_Estimate),c("SNP","CHR","MAF","p_adj")] %>% 
      slice_head(n = 200) %>%
      mutate(i=row_number()) %>%
      select(i,SNP,CHR,MAF,p=p_adj) %>%
    gt() %>% 
    fmt_scientific(columns = vars(p), decimals = 2) %>%
