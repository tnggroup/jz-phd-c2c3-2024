---
title: "JZ_GED_PHD_C1 Analysis Setup 4"
...
project$sumstats$effect.logit[which(project$sumstats$code=="ADHD05")]=T #Column OR. Munge non-OR detected. Consistent with non-OR.
...
project$sumstats$effect.logit[which(project$sumstats$code=="AUTI07")]=T #Assuming log(OR) since not plain OR
project$sumstats$effect.logit[which(project$sumstats$code=="BIPO02")]=T #Assuming log(OR) since not plain OR
...
project$sumstats$effect.logit[which(project$sumstats$code=="DEPR08")]=F #https://www.med.unc.edu/pgc/files/2019/04/PGC_MDD_2018_README_third_180316.pdf # As DEPR05
...
project$sumstats$effect.logit[which(project$sumstats$code=="INCO03")]=F #As per munge
project$sumstats$effect.logit[which(project$sumstats$code=="INSO02")]=F #As per munge
project$sumstats$effect.logit[which(project$sumstats$code=="MIGR01")]=F #As per munge
project$sumstats$effect.logit[which(project$sumstats$code=="NEUR01")]=F #The official read-me states "regression beta"
project$sumstats$effect.logit[which(project$sumstats$code=="RISK02")]=F #As per munge
project$sumstats$effect.logit[which(project$sumstats$code=="RISK03")]=F #As per munge
project$sumstats$effect.logit[which(project$sumstats$code=="SCHI04")]=F #OR, as per munge
...
project$sumstats$se.logit[which(project$sumstats$code=="ADHD05")]=T #consistent with log(OR) above
...
project$sumstats$se.logit[which(project$sumstats$code=="ANXI03")]=F #Assume linear becasue of Kirstin Purves assuring that both were converted to linear scale. Consistent with munge non-OR.
project$sumstats$se.logit[which(project$sumstats$code=="AUTI07")]=T #Assuming log(OR) since not plain OR
project$sumstats$se.logit[which(project$sumstats$code=="BIPO02")]=T #Assuming log(OR) since not plain OR
project$sumstats$se.logit[which(project$sumstats$code=="DEPR05")]=T #log(OR) standard errors as of documentation
project$sumstats$se.logit[which(project$sumstats$code=="DEPR08")]=T #Assuming log(OR) standard errors as DEPR05
project$sumstats$se.logit[which(project$sumstats$code=="HEAL01")]=F #Assume linear scale because of linear beta
project$sumstats$se.logit[which(project$sumstats$code=="INCO03")]=F #Assume linear scale because of linear beta
project$sumstats$se.logit[which(project$sumstats$code=="INSO02")]=T #Assuming log(OR) standard errors as some of the datasets above.
project$sumstats$se.logit[which(project$sumstats$code=="MIGR01")]=F #Assume linear scale because of linear beta
project$sumstats$se.logit[which(project$sumstats$code=="NEUR01")]=F #Assume linear scale because of linear beta
project$sumstats$se.logit[which(project$sumstats$code=="RISK02")]=F #Assume linear scale because of linear beta
project$sumstats$se.logit[which(project$sumstats$code=="RISK03")]=F #Assume linear scale because of linear beta
project$sumstats$se.logit[which(project$sumstats$code=="SCHI04")]=T #Assuming log(OR) standard errors as some of the datasets above.
...
#project$sumstats.sel.code<-c("ANXI03","DEPR05","DEPR08")
...
project$sumstats.sel[,c("code","name","name.nice","name.nice.and_code", "year","n_case","n_control","n_total","pmid","reference_doi","samplePrevalence","populationPrevalence","effect.logit","se.logit","dependent_variable.OLS","mungedpath")]
...
project$printCorr <- function(corr, SE=NULL, filename, addrect = 4, is.corr = T, number.cex = 1.5, number.digits=2){
...
project$printCorrSimplified(
  corr = abs(project$mvLD$covstruct.mvLDSC.1kg$S_Stand.forPlot),
  #SE = project$mvLD$covstruct.mvLDSC.1kg$S_Stand.SE,
...
project$printCorr(
  corr = project$mvLD$covstruct.mvLDSC.1kg$S_Stand,
  SE = project$mvLD$covstruct.mvLDSC.1kg$S_Stand.SE,
  ...
  for(iFactorConfiguration in 1:length(project$CFA$nFactors)) {
    ...
    for(nSessioPattern in 1:sessionPatternLength){
      ...
      project$CFA$models[nModel,c("nModel")]<-nModel
        ...
        #code
      project$CFA$models[nModel,c("code")]<-paste0("M",project$CFA$nFactors[iFactorConfiguration],"-",project$CFA$nIndicators,
